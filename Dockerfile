...
COPY data/ ./data/
# Self-hosted display/body/mono woff2 (Unbounded + IBM Plex) imported by
# pages/_app.tsx via next/font/local ("../fonts/*.woff2"). MUST be copied or
# `yarn build` fails with "Module not found: Can't resolve '../fonts/...'".
# (Root cause of failed CI run #4 / commit 6e987fe8 — 2026-09-10.)
COPY fonts/ ./fonts/
...
# Runtime-decided feature flags (helpers/runtimeFlags.ts). The browser now reads
# these from __NEXT_DATA__ (server-provided at request time), so build-time values
# only matter for statically generated pages (blog/[slug]). Declared here as a
# safety net so a build-arg is never silently dropped — an undeclared
# NEXT_PUBLIC_SHOW_SOCIAL_LINKS inlined as undefined vs "false" at runtime was the
# 2026-09 hydration-mismatch (React #418/#423 → blank landing page on phones).
ARG NEXT_PUBLIC_SHOW_SOCIAL_LINKS=
ENV NEXT_PUBLIC_SHOW_SOCIAL_LINKS=${NEXT_PUBLIC_SHOW_SOCIAL_LINKS}
ARG NEXT_PUBLIC_ENABLE_PRODUCT_CATALOG=
ENV NEXT_PUBLIC_ENABLE_PRODUCT_CATALOG=${NEXT_PUBLIC_ENABLE_PRODUCT_CATALOG}
ARG NEXT_PUBLIC_INLINE_TIP_CHECKOUT=
ENV NEXT_PUBLIC_INLINE_TIP_CHECKOUT=${NEXT_PUBLIC_INLINE_TIP_CHECKOUT}
ARG NEXT_PUBLIC_CLEAN_CHECKOUT_V2=
ENV NEXT_PUBLIC_CLEAN_CHECKOUT_V2=${NEXT_PUBLIC_CLEAN_CHECKOUT_V2}
ARG NEXT_PUBLIC_CHECKOUT_SWR=
ENV NEXT_PUBLIC_CHECKOUT_SWR=${NEXT_PUBLIC_CHECKOUT_SWR}
...
# Build ARG so constrained builders (the 2-vCPU droplet) default to 2, while CI
# runners (GitHub-hosted = 4 vCPU / 16 GB) can pass --build-arg NEXT_BUILD_CPUS=4
# to speed up static page generation.
