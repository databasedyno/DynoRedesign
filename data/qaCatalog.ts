/* Auto-extracted QA catalog (shared by /QA and /quality). Pure data — no imports. */
...
export const TEST_SECTIONS: TestSection[] = [
  /* ============================================================
     SECTION 0: END-TO-END USER JOURNEYS (whole-app flows)
     These stitch multiple features into the real paths a user takes.
     Run them first — a broken journey means a broken product even if
     each feature passes in isolation.
     ============================================================ */
  {
    id: "journeys",
    ...
    icon: "🧭",
    description:
      "Complete, cross-feature flows a real user follows — from the first click to money moving and reporting. Test these top-to-bottom; note the exact step where a journey breaks.",
    cases: [
      {
        id: "JRN-01",
        title: "New Merchant → First Live Payment (golden path)",
        priority: "Critical",
        preconditions:
          "Fresh signup — use a qa_-prefixed throwaway email. SAFE MODE: do NOT complete a real on-chain payment on the production DB — use the sandbox (/pay/demo) for the buyer leg.",
        steps: [
          { id: "1", action: "Sign up with email at /auth/register, verify the email OTP", expected: "Account created. Lands on the Dashboard with the onboarding / get-started checklist." },
          { id: "2", action: "Create your first company (name, country)", expected: "Company created and now selectable in the top-bar company switcher." },
          { id: "3", action: "Open KYC (Dashboard banner / Company → Verification)", expected: "KYC status is visible and moves not_started → pending after starting Veriff." },
          { id: "4", action: "Add a payout wallet address for one coin (OTP-verified)", expected: "Address saved and listed under Money → Payout wallets. A blank label auto-names it." },
          { id: "5", action: "Create a payment link (amount, currency, accepted coins)", expected: "Success modal with a shareable URL + QR. Only coins with a configured wallet are offered." },
          { id: "6", action: "Open the link as a buyer (private window or sandbox)", expected: "Checkout shows merchant name, amount, fee breakdown and the coin choices." },
          { id: "7", action: "Pay in the sandbox", expected: "Status flows waiting → confirming → completed. Buyer sees a success screen + receipt." },
          { id: "8", action: "Return to the app", expected: "The payment appears in Money → Transactions; Dashboard revenue/counters update; a receipt/invoice is generated." },
          { id: "9", action: "(If a webhook is configured) check your receiver", expected: "A payment.confirmed/settled webhook is delivered." },
        ],
        notes: "This is the money path. Any blocking step = the merchant cannot get paid — treat as Critical.",
      },
      {
        id: "JRN-02",
        title: "Buyer Checkout — Happy Path (buyer's perspective)",
        priority: "Critical",
        preconditions: "Any active payment link, or the sandbox /pay/demo.",
        steps: [
          { id: "1", action: "Open the checkout URL", expected: "Loads with merchant branding, amount, fee breakdown and supported coins." },
          { id: "2", action: "Pick a coin (e.g., BTC)", expected: "Payment address, crypto amount and QR shown; the countdown timer starts." },
          { id: "3", action: "Copy the address / scan the QR", expected: "Address matches the QR; crypto amount is correct to the right number of decimals (shown as a string — no float drift)." },
          { id: "4", action: "Send the exact amount (sandbox)", expected: "Status: waiting → confirming (with confirmation count) → completed." },
          { id: "5", action: "Reach the success screen", expected: "Shows amount, transaction reference and merchant; the receipt link works." },
          { id: "6", action: "Switch the checkout language", expected: "All buyer-facing copy translates; amounts stay correct and locale-formatted." },
        ],
      },
      {
        id: "JRN-03",
        title: "Developer / API Integration (create → webhook → reconcile)",
        priority: "High",
        preconditions:
          "An x-api-key (Settings → Developers → API keys). Use Swagger (/api/docs), curl or Postman, plus a webhook receiver such as webhook.site.",
        steps: [
          { id: "1", action: "Create an API key", expected: "The secret is shown ONCE; copy it. A blank name auto-generates one." },
          { id: "2", action: "Save a webhook URL (Developers → Webhooks)", expected: "A whsec_… signing secret is auto-generated and shown once." },
          { id: "3", action: "POST /api/user/createPayment (or cryptoPayment) with your key", expected: "200 with a first-class payment object (id, status, hosted_url or crypto details)." },
          { id: "4", action: "Settle the payment (sandbox)", expected: "Your webhook receiver gets the event." },
          { id: "5", action: "Verify the X-Dynopay-Signature-V2 header over the RAW body with your whsec_", expected: "Signature matches. An endpoint with NO secret receives the payload unsigned (no default signature)." },
          { id: "6", action: "GET /api/user/events?limit=&starting_after=", expected: "Your delivery is listed with status; paginate using next_cursor until has_more is false." },
          { id: "7", action: "POST /api/user/events/{id}/resend for a delivery", expected: "The event is re-delivered with a fresh webhook_id, timestamp and signature." },
        ],
        notes: "Covers the webhook hardening: unsigned-when-no-secret, V2 signature over raw bytes, 24h retry schedule, and the events + resend reconciliation API.",
      },
      {
        id: "JRN-04",
        title: "Payment Edge Cases — Underpaid / Overpaid / Expired / Failed",
        priority: "Critical",
        preconditions: "Use /pay/payment-states-demo for the visual states and the sandbox for behaviour.",
        steps: [
          { id: "1", action: "Underpaid: send less than due", expected: "'Underpaid' state shows amount received vs expected and an option to send the remainder." },
          { id: "2", action: "Overpaid: send more than due", expected: "'Overpaid' state shows the overpayment; merchant is notified." },
          { id: "3", action: "Expired: let the timer reach 0", expected: "'Expired' message with a retry / new-address path." },
          { id: "4", action: "Failed: trigger a failed payment", expected: "Clear error with support / retry options." },
          { id: "5", action: "Check the merchant side for each", expected: "Transactions reflects the correct status and the matching webhook event fires." },
        ],
      },
      {
        id: "JRN-05",
        title: "Account Recovery — Forgot Password + 2FA + Backup Code",
        priority: "High",
        preconditions: "A test account with 2FA enabled (authenticator app + saved backup codes).",
        steps: [
          { id: "1", action: "Forgot password → enter email", expected: "Reset email / OTP sent (in SAFE MODE the OTP is surfaced for testing)." },
          { id: "2", action: "Use the reset link / OTP → set a new password", expected: "Password updated; success message." },
          { id: "3", action: "Log in with the new password", expected: "Email OTP step, then a 2FA prompt appears." },
          { id: "4", action: "Enter the TOTP code", expected: "Login completes." },
          { id: "5", action: "Log in again using a backup code instead of TOTP", expected: "Login succeeds; that backup code is consumed and cannot be reused." },
          { id: "6", action: "Regenerate backup codes", expected: "Old codes are invalidated; a fresh set is shown." },
        ],
      },
      {
        id: "JRN-06",
        title: "Auto-Convert to Stablecoin (settlement flow)",
        priority: "Medium",
        preconditions: "A company with auto-convert available (Settings → Payments).",
        steps: [
          { id: "1", action: "Enable auto-convert to USDC/USDT", expected: "Setting saved." },
          { id: "2", action: "Receive / settle a crypto payment (sandbox)", expected: "A conversion record is created (source → target, rate captured)." },
          { id: "3", action: "Check Dashboard / Conversions", expected: "Conversion status shown; on success the stablecoin balance reflects it." },
          { id: "4", action: "Force a failed conversion, then Retry", expected: "Retry succeeds or records a new attempt with status." },
        ],
      },
      {
        id: "JRN-07",
        title: "Referral Program — End to End",
        priority: "Medium",
        steps: [
          { id: "1", action: "Grab your referral link (Grow → Refer & earn)", expected: "A copyable code / link." },
          { id: "2", action: "Sign up a new merchant via the ?ref= link (or enter the code at register)", expected: "Code validates; the referral discount is applied to the new account." },
          { id: "3", action: "As the referrer, view your referrals", expected: "The referred user and pending earnings appear." },
          { id: "4", action: "As the referred merchant, open fee preview", expected: "The referral discount is reflected for its active window." },
          { id: "5", action: "Check the leaderboard", expected: "The new referral is counted." },
        ],
      },
      {
        id: "JRN-08",
        title: "Admin Operations — Withdrawal Approval",
        priority: "High",
        preconditions: "Super-admin login (/admin/login).",
        steps: [
          { id: "1", action: "A merchant requests a payout/withdrawal", expected: "It appears in Admin → Transactions/Withdrawals as pending." },
          { id: "2", action: "Approve it", expected: "Status → processed; merchant notified; balances update." },
          { id: "3", action: "Reject a different request", expected: "Status → denied with a reason; merchant notified." },
          { id: "4", action: "Check the admin overview", expected: "Platform balances / fee revenue reflect the changes." },
        ],
      },
    ],
  },
