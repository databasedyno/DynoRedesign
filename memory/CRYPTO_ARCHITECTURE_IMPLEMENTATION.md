...
| 4 | **KYC/AML activation** (Veriff real keys) | 2 | 🚧 **ACTIVATED IN PREVIEW (2026-08-21), prod rollout pending** — real Veriff creds wired; session-create verified (live 201 from stationapi.veriff.com); webhook hardened (raw-body HMAC + x-auth-client, CSRF-exempt, idempotent) verified 4/4; `/kyc/complete` redirect page added; hostbay exempt via `KYC_EXEMPT_COMPANY_IDS`/`KYC_EXEMPT_USER_IDS`. Remaining: push code to `Improvement` + flip DO env (real VERIFF_* + exempt vars) + set Veriff Station "Webhook decisions URL". AML address-screening (Chainalysis/TRM) deferred per user. | E1 | 90-day #2 |
