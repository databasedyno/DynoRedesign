# === 2026-06 (fork) NOTE — "Save to GitHub won't commit" = NOT preflight checks ===
# User reported Save-to-GitHub failing. Reproduced the full .husky/pre-commit locally against all 334
# staged files → EXIT 0: preflight-tsc (backend+frontend tsc OK), check-secrets OK, check-email-dark-mode
# OK, check-contrast is warn-only (|| true, never blocks). No secrets in current tree OR across all 33
# commits of history; no .env tracked/staged (all gitignored); largest tracked file 3.5MB (<100MB). Local
# commits succeed (33 checkpoints in git log). => Failure is in the platform "Save to GitHub" PUSH step,
# not the code. Likely causes (per support KB): GitHub OAuth needs reconnect, no repo write access, plan
# requirement, or a diverged-branch conflict. Awaiting user's exact dialog error to pinpoint.
# ============================================================================================


# === 2026-06 (fork) DONE — LEAKED WEBHOOK PLACEHOLDER CLEARED FOR 32 PROD BRANDS ===
# STATUS: DONE & VERIFIED (SQL). User approved clearing the leaked frontend placeholder
#   'https://mystore.com/dynopay-webhook' (WebhookNotificationsSection.tsx initial state — fixed
#   last session) that was persisted to tbl_company.webhook_url but never explicitly set by merchants.
# ACTION: backend/scripts/clear_leaked_webhook_url.js (transactional) set webhook_url = NULL on the
#   exactly-32 rows still carrying that placeholder. company_ids: 38,45,54,69,71,73,79,80,86,90,91,92,
#   94,101,102,105,123,124,128,134,140,142,144,147,150,178,182,187,189,210,221,231. webhook_events was
#   already NULL for all 32 (no events configured). The Dev Store (company 1) was cleared last session.
# VERIFY: `select count(*) from tbl_company where webhook_url='https://mystore.com/dynopay-webhook'` = 0.
# BACKUP: /app/memory/backups/webhook_url_leak_backup_20260915_161720.json (pre-mutation snapshot).
# Recurrence prevented by the FE fix (placeholder removed from component initial state).
# ============================================================================================
...
<!-- 2026-09-02 (fork, pod vault-setup): LANDING "SMALL→BIG" ROOT-CAUSE FIX + STATUS BADGES + WALLET FLOW —
     ...
     eslint 0 errors). LIVE prod DB, SAFE MODE. Preview: https://passphrase-init.preview.emergentagent.com
...
<!-- 2026-09-02 (fork, pod 054d2272 / preview setup-credentials): FIRST+LAST NAME COLLECTION +
  ...
  NOT E2E-TESTED (live prod DB + email off make it unsafe/infeasible): the register name step
      ...
      screenshot-verified. Preview host: https://passphrase-init.preview.emergentagent.com
...
<!-- 2026-09-01 (fork, pod e952fc3d): EMAIL-ARCHITECTURE FRONTEND + REWARD CTA + CHAIN HUB LINKS — DONE + VERIFIED. LIVE prod DB, SAFE MODE. Preview: https://passphrase-init.preview.emergentagent.com
...
<!-- 2026-06 (fork): LANDING POLISH x3 (trust strip + checkout-story animation + dark-mode terminal) — DONE + VERIFIED (screenshot light+dark, tsc 0). Frontend-only, additive, no money-path touched. Preview: https://passphrase-init.preview.emergentagent.com (Next.js app is at /app ROOT; /app/frontend is a DIFFERENT unused dir — v3 components live in /app/Components/Page/Home/v3/).
...
Preview: https://passphrase-init.preview.emergentagent.com (LIVE prod DB, SAFE MODE). Frontend tsc EXIT 0.
...
Preview: https://passphrase-init.preview.emergentagent.com · Login: onarrival21@gmail.com / Katiekendra123@ (LIVE prod DB, SAFE MODE, EMAIL OFF).
...
Preview: https://passphrase-init.preview.emergentagent.com · Login: onarrival21@gmail.com / Katiekendra123@ (LIVE prod DB, SAFE MODE, EMAIL OFF). FE tsc EXIT 0, BE healthy.
...
## Wave 4a — shared email template (2026-06) — DONE (tsc 0, dark-mode guard OK, harness 138/138)
- Footer on EVERY email: "You're receiving this because … ({{email}})" + "Manage email preferences" (→ /settings?section=notifications) for merchants; buyer variant (non-custodial note, no prefs link); admin variant; legal line "© {{year}} {{EMAIL_LEGAL_NAME}}" (env, default "Dynopay Payments Ltd.", optional EMAIL_LEGAL_ADDRESS). Recipient e-mail injected at transport (`%%TO_EMAIL%%` swapped in mailTransporter). `audience` option on baseEmailTemplate/dynoPayEmailTemplate/dynoPayGreetingTemplate; buyer senders tagged (receipts, orders, reminders, nudges, refunds, subscriptions, payment request), admin senders tagged.
- Preheader fallback = heading (0 emails without preheader). Dates: `emailDateParts()`/`formatEmailTime` (UTC, "5 June 2026 · 14:02 UTC") replaces 14 ad-hoc toLocale* sites; "at" separator → "·"; invoice/expiry/freeze dates via formatEmailDate/formatEmailDateTime.
- CTA deep-links: KYC → /kyc, tier upgrade → /settings?section=plan, auto-convert emails → /payouts, withdrawal → /payouts, subscriptions → /pay-links, webhooks → /developer-keys?tab=webhooks, activation gates → /wallet | /kyc | /settings?section=company, pending/confirming/partial → /transactions?search=<tx>.
- Network vocabulary: `assetNetworkLabel()` (utils/networkLabels) in wallet + refund emails; refund emails no longer print the raw explorer URL.
- i18n: chrome.legal/whyMerchant/whyBuyer/whyAdmin/managePrefs ×6 (scripts/inject_footer_chrome_i18n.py).

## Wave 4b/4c/4d — money emails, new emails + controls, merges (2026-06) — DONE (testing_agent iteration_179: 100% backend + frontend)
- 4b: Pending email → network, confirmations needed, typical wait; Partial (merchant) → fiat ≈ + network; buyer underpaid nudge → network; Partial-expired (both branches in paymentController.processIncompletePayments) → full money path via renderMoneyPath; Withdrawal success → explorer link + network; Auto-convert payout → settlement wallet (masked) + chain + withdrawal-tx explorer link; customer receipt → asset·network; refund emails → original payment ref (+ merchant gas funded), no raw explorer URL; order emails → "Paid with" crypto row, merchant sale email points to the Payment-settled email for fee/net/wallet, button CTA, shipping country fix.
- 4c: NEW `services/email/payoutEmails.ts` sendPayoutDelayedEmail (stage delayed|failed, reason keys network/exchange/review/unknown, 6 langs) wired: conversionService.notifyConversionFailed → merchant 'failed'; notifyStalledConversions() (in-flight > 2h, redis guard) → 'delayed'; merchantPoolMonitoring recovery stuck > 2h → notifyMerchantSettlementDelayed. Overpaid: overpaymentNotifier refactored into buildOverpaidMerchantEmail (refund CTA → /transactions?search=, network, customer) + NEW buildOverpaidBuyerEmail (amount due/sent/excess, merchant contact mailto CTA, buyer footer). Confirming emails OPT-IN: company notification_prefs.categories.confirming (default false; whitelist + isConfirmingEmailOptedIn gate in pendingPaymentService; FE switch data-testid company-category-confirming-switch in Settings → Notifications → Email categories; i18n ×6).
- 4d: retired per-login known-device email (new-device alert kept), large-transaction alert folded into Payment settled as "Large payment" badge/subject (≥ $1,000), admin per-visit e-mail retired (POST /api/track/visitor is a 200 no-op), 4 wallet-OTP senders are thin wrappers over sendStepUpCodeEmail (scopes wallet/withdrawal/exchange), weekly payout digest gained gross/fees/net rows + Receipts & Tax CSV statement link + CTA /transactions?range=7d.
- Harness now 141 renders; gallery/report regenerated. Next: Wave 5 (payment result pages, checkout trust cues, invalid-token states) → Wave 6 → 7 → 8; parked Wave 2–3 dashboard regression sweep.

## "Wallet" → "Payout address" rename + Wave 5 completion — 2026-06 — DONE (testing_agent iteration_180: 100% backend + frontend)
- User question answered: every "wallet" email (added/updated/deleted, update/delete OTP, /wallet-security undo link, batch summary) goes to the MERCHANT — "wallet" meant their non-custodial payout/forwarding destination. Buyers never receive wallet emails.
- Rename (user chose "rename everywhere", URLs unchanged): `scripts/i18n/rename_wallet_to_payout_address.py` (idempotent; EN phrase rules + de/es/fr/pt/nl noun+agreement rules; report `plan/audit/wallet_rename_report.md`, ~1,460 strings). Surfaces: sidebar/mobile nav/command palette, /wallet page + manager/add/delete modals, wallet-security (Settings → Security), Payouts (Settlement addresses / By payout address), Get-started, dashboard attention items, create-pay-link gates, referrals, team/activity log, 2FA reset copy, all merchant emails (backend/locales/*/emails.json + walletEmails/walletSecurityEmails/referralEmails/conversionEmails hardcoded strings), in-app notifications (walletChangeAlert.ts), merchant API error messages (merchantApiRouter, walletBatch, addressBook, walletOtp, publishableKey/elements). KEPT on purpose: buyer wallet ("Open in wallet app", network-fee line), Customer Wallets API/docs, admin platform wallets, marketing "your own wallet / wallet you control" lines, legal docs. Field labels: "Wallet Name" → "Label", "Wallet Address" → "Address".
- Wave 5 finish: receipt page `/receipt/<token>` → Print button (print CSS hides actions), refund/what-to-do note, merchant-contact block (legal name, email, website, phone, address — live from tbl_company via `loadMerchantContact`, hidden when the brand has no contact data); order page `/order/<ref>` → merchant email (mailto with order ref), website link, "View payment receipt" (receipt looked up via payment-link transaction_reference → tbl_payment_receipt). New receipt.* labels ×6 langs; landing.json order.helpLineWebsite/viewPaymentReceipt ×6. Already shipped earlier in Wave 5: rate-lock tiers amber ≤5 min / red ≤2 min / urgent ≤60 s + "Refresh quote" on expiry, /payment/verify|success|failed states, /unsubscribe invalid-token help, checkout non-custodial + fee-disclosure lines, success card (return to merchant, receipt download/link, save merchant, email catch).
- Verified: FE tsc 0, BE tsc 0, dark-mode guard OK, harness 141/141, eslint clean on touched files; testing_agent iteration_180 100%. Note for QA: `/wallet-security` is the public undo-link landing; the merchant security UI lives in Settings → Security.
- Next: Wave 6 creator pages (tip block, supporter wall, store grid), Wave 7 marketing (/about legitimacy, /fees worked example), Wave 8 in-app pages; parked Wave 2–3 dashboard regression sweep.

## Wave 6 creator pages + Waves 2–3 regression sweep — 2026-06 — DONE (testing_agent iteration_181: 100% backend + frontend)
- User choices: presets default **$10 / $25 / $50** (platform $10 floor kept — $5 would be hidden), supporter wall **opt-in, off by default**, stored in NEW nullable column `support_widget_show_wall` on tbl_user + tbl_company via boot migration `0028_support_widget_show_wall` (applied on the prod DB at backend restart: "1 applied"); phone sticky buy bar added to product pages.
- Backend: `paymentLinkController` public creator endpoint → `support_widget.show_wall` + `recent_supporters` (last 8 completed tips via `getRecentSupporters(jar)`, first-name-only, anonymous → name null); `creatorProfile.ts` GET/PUT `support_widget_show_wall`; `storefrontScope.ts` column lists; models default presets [10,25,50].
- Frontend: `Components/Page/Creator/SupporterWall.tsx` (creator-supporter-wall / creator-supporter-row) rendered inside SupportWidget when on; Tips settings toggle `support-widget-show-wall` + hint; `CreatorLivePreview` shows `preview-supporter-wall` sample rows live; `creator-noncustodial-note` ("Paid straight to {brand} — Dynopay never holds your funds.") under Powered by; Shop: 4 columns at ≥lg (`lg={3}`), type + category chips only when >8 products (`shop-toolbar-types/-categories`), search box `shop-search-input` when >20 products (title/subtitle/category), MiniCart FAB unchanged; product page `product-sticky-buy-bar` (xs/sm only: title, live price × qty, add-to-cart icon, Buy now) + bottom padding. i18n ×6: landing.json creator.wall.*, creator.nonCustodial, shop.searchPlaceholder; common.json storefront.form.showWall/showWallHint, storefront.preview.wallTitle.
- Waves 2–3 dashboard regression sweep run in the same iteration_181 → both marked SHIPPED in plan/wave_execution_plan.md.
- Housekeeping: testing agent had left the user's account language on `de` (iteration_180 language test) → restored to `en` via PUT /api/user/profile; supporter wall flag restored to OFF after the test. The Dev Store brand still has no contact email/website/address — user will add them in Settings → Brand so receipts/order pages show the contact block.
- Follow-ups noted by QA (not blocking): paylink rows use a single `data-testid="paylink-row"` (consider per-id); a React validateDOMNesting <div> in <p> warning on a dashboard page (lint-level).
- Next: Wave 7 marketing pages (/about legitimacy block, /fees worked example, landing trust strip, docs phone nav) → Wave 8 in-app pages (create-pay-link single form, product editor live preview, KYC status timeline) → cross-wave Part F checks (WCAG AA, CWV, non-English samples).

## Wave 7/8 picks + tip thank-you card — 2026-06 — DONE (testing_agent iteration_182: 7/8 → download bug fixed & self-verified via Playwright expect_download)
- **/about legitimacy block** (`Components/Page/About/LegitimacyBlock.tsx`, `about-legit-*`): legal entity "Dynopay Payments Ltd.", operating since 2024, contact support@dynopay.com (user chose to publish only existing facts — no registration no./address yet), custody line, links Terms/Privacy/AML/System status, non-custodial note. i18n landing.json `about.legit.*` ×6.
- **/fees worked example** (`Components/Page/Fees/WorkedExample.tsx`, `fees-worked-*`): "$100 sale → you receive $X" = 100 − tier % − live network fee for the chosen payout coin (tier chips Starter/Growth/Scale/Enterprise, coin select). Per user: the $1 fixed per-payment fee is intentionally NOT shown; footnote says exact fee appears on every invoice. fees.json `worked.*` ×6.
- **Create-pay-link single form** (`Components/Page/CreatePaymentLink/index.tsx`): 01 How much (amount+currency) → 02 What for (product picker + description) → collapsed "More options" now holds the coin picker (`pay-link-more-coins`, no step badge) + Details (`pay-link-more-details`: client name, expiry, fee payer) + customer email/tax/webhooks. Default coins = ALL coins with a payout address (was BTC/ETH/USDT-ERC20). Coin blocker auto-opens the accordion. Donation links keep the old layout. `PaymentSettingsBasic` got `part`/`amountStep`/`detailsStep`; `CryptoSelection` got `step`. Primary submit now `data-testid=pay-link-create-btn`. createPaymentLinkScreen.json `advancedOptionsHintSingleForm(Edit)` ×6.
- **Tip thank-you card** (`Components/Page/Creator/TipThankYouCard.tsx`): 1080×1080 canvas PNG (creator accent gradient, avatar/initial, "I just supported", name, amount, message, QR + URL, powered-by) rendered in the InlineTipCheckout confirmed phase for `mode='tip'`; Download (blob anchor) + Share (navigator.share with file → share text/url → X intent + download fallback). SupportWidget/InlineTipCheckout accept `accentColor` (CreatorProfile passes the theme accent). QA demo route `/pay/tip-card-demo` (allow-listed in middleware.ts like the other demo pages). landing.json `creator.card.*` ×6. Bug found by QA and fixed: `toBlob` wrapper resolved null via `??` on a void call — download/share-file never fired.
- Next: Wave 7 landing trust strip + docs phone nav; Wave 8 product editor live preview + KYC status timeline; Part F (WCAG AA contrast/focus, CWV, non-English samples). User still to add brand contact details in Settings → Brand.

## Wave 7 finish + Wave 8 product preview + tip goal bar — 2026-06 — DONE (testing_agent iteration_183: 100% backend + frontend)
- **Landing trust strip** (`Components/Page/Home/v5/HeroTrustStrip.tsx`, `hero-trust-*`): 4 pills under the hero CTAs — Non-custodial → /about#about-legitimacy, 15 coins → /fees, Fees from 0.5% → /fees, Since 2024 → /about; live proof strip kept below. landing.json `v5.hero.trust.*` ×6.
- **Monthly tip goal bar**: new nullable NUMERIC(12,2) `support_widget_monthly_goal` on tbl_user + tbl_company (boot migration `0029_support_widget_monthly_goal`, applied). Backend: creatorProfile GET/PUT (10..10,000,000 or null), `getDonationAggregatesSince(jar, monthStartUTC)` → `support_widget.monthly_goal` + `month_raised`. Frontend: `TipGoalBar.tsx` (`tip-goal-bar/-label/-pct/-footer`, progressbar aria) in SupportWidget; Tips settings number input `support-widget-monthly-goal` (min-snap 10, empty = off); live preview `preview-tip-goal` (42% sample). i18n landing `creator.goal.*`, common `storefront.form.monthlyGoal*`, `storefront.preview.goalProgress` ×6.
- **Docs phone nav** (`pages/documentation.tsx`): sticky `docs-mobile-nav` (current section + `docs-mobile-sections-btn`) → bottom `Drawer` `docs-mobile-sheet` with search + every section/endpoint (`docs-sheet-section-*`, `docs-sheet-endpoint-*`, aria-current); `SectionPager` Prev/Next (`docs-pager-<id>`) injected at the end of all 18 sections (mobile only).
- **Product editor live preview**: `ProductEditorWithPreview.tsx` (grid, `product-preview-toggle` on phones, `product-preview-desktop` sticky ≥lg) + `ProductLivePreview.tsx` (`product-live-preview-*`: cover, Digital/Draft badges, title, subtitle, price w/ currency symbol, 3-line description, Buy now); `ProductEditor` emits `onDraftChange`. Used by /pay-links/products/new and /[productId]/edit. common.json `productEditor.preview.*` ×6.
- QA note (not blocking): sticky `main-page-header` can intercept MUI Select clicks on the product editor at some scroll offsets — consider z-index review.
- Waves 1–7 now COMPLETE; Wave 8 remaining: KYC status timeline; then Part F cross-wave checks.
