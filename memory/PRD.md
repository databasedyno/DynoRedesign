...
- All app URLs → preview URL `https://multi-chain-checkout-6.preview.emergentagent.com`; preview host FIRST in `CORS_ALLOWED_ORIGINS` (+ dynopay.com + checkout.dynopay.com)
...
- Re-provisioned per documented procedure on fresh container; preview URL now https://multi-chain-checkout-6.preview.emergentagent.com; safety overrides re-applied (NODE_ENV=production, WORKER_ROLE=secondary, ENABLE_BACKGROUND_JOBS=false); next build standalone; all health checks green.
