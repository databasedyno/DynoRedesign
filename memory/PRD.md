...
- All app URLs → preview URL `https://blockchain-pay-29.preview.emergentagent.com`; preview host FIRST in `CORS_ALLOWED_ORIGINS` (+ dynopay.com + checkout.dynopay.com)
...
- Re-provisioned on fresh container (preview https://blockchain-pay-29.preview.emergentagent.com); NOTE: run the two yarn installs SEQUENTIALLY (parallel installs corrupt the shared yarn cache with ENOENT .yarn-metadata.json). Safety overrides re-applied + verified; next build standalone; all health checks green.
...
- Re-provisioned per documented procedure on fresh container; preview URL now https://blockchain-pay-29.preview.emergentagent.com; safety overrides re-applied (NODE_ENV=production, WORKER_ROLE=secondary, ENABLE_BACKGROUND_JOBS=false); next build standalone; all health checks green.
