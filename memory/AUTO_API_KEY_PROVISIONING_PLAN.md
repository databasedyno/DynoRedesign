...
> **Env verified:** setup done on preview `https://speedup-check.preview.emergentagent.com` (backend :3300 healthy, frontend :3000 200, csrf 200, login 401 bad-creds, Railway PG + Redis + Tatum connected, background_jobs.eligible=false — see §Environment).
...
- **Preview URL:** `https://speedup-check.preview.emergentagent.com` (=`preview_endpoint`)
