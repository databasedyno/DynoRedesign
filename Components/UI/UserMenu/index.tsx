...
export default function UserMenu() {
  ...
  const menuItems = () => Array.from(menuRef.current?.querySelectorAll<HTMLElement>('[role="menuitem"], button, a[href]') || []);
  // Keyboard menu pattern: focus lands on the first item, arrows cycle, Escape returns to the trigger.
  useEffect(() => {
    if (anchorEl) requestAnimationFrame(() => menuItems()[0]?.focus());
  }, [anchorEl]);
  ...
  const activateOnEnter = (e: React.KeyboardEvent<HTMLElement>) => {
