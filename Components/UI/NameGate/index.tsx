...
const NameGate: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (!first) { setError(t("nameGate.firstRequired", { defaultValue: "First name is required" })); return; }
    if (!last) { setError(t("nameGate.lastRequired", { defaultValue: "Last name is required" })); return; }
    setError("");
    setSaving(true);
    try {
      ...
      setError(err?.response?.data?.message || t("nameGate.saveFailed", { defaultValue: "Couldn't save your name. Please try again." }));
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      <Typography sx={{ fontWeight: 700, fontSize: "20px", color: "text.primary", fontFamily: "var(--font-sans)" }}>
        {t("nameGate.title", { defaultValue: "What's your name?" })}
      </Typography>
      <Typography sx={{ fontSize: "14px", color: "text.secondary", fontFamily: "var(--font-sans)", mt: 0.75, lineHeight: 1.5 }}>
        {t("nameGate.body", { defaultValue: "Add your first and last name so we can personalize your account, receipts and payout emails." })}
      </Typography>
      ...
      <Box sx={{ mt: 2.5, display: "flex", flexDirection: "column", gap: 1.5 }}>
        <Box sx={{ display: "flex", gap: 1.5, flexDirection: isMobile ? "column" : "row" }}>
          <Box sx={{ flex: 1 }}>
            <InputField
              ...
              label={t("nameGate.firstName", { defaultValue: "First name" })}
              placeholder={t("nameGate.firstName", { defaultValue: "First name" })}
            />
          </Box>
          <Box sx={{ flex: 1 }}>
            <InputField
              ...
              label={t("nameGate.lastName", { defaultValue: "Last name" })}
              placeholder={t("nameGate.lastName", { defaultValue: "Last name" })}
            />
          </Box>
        </Box>
        {error && (
        ...
        <CustomButton
          ...
          variant="primary"
          size="medium"
          label={saving ? t("nameGate.saving", { defaultValue: "Saving..." }) : t("nameGate.saveContinue", { defaultValue: "Save & continue" })}
          onClick={handleSave}
          disabled={saving}
          fullWidth
