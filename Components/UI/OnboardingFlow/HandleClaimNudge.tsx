...
const HandleClaimNudge: React.FC = () => {
  ...
  const formatError = useMemo(() => {
    ...
    if (!HANDLE_RE.test(handle)) {
      return t("handleNudge.formatError", { defaultValue: "3–30 chars: lowercase letters, numbers, - or _" });
    }
    return null;
  }, [handle, t]);
  ...
  const claim = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: TOAST_SHOW,
        payload: {
          message: t("handleNudge.reservedToast", { url: `${siteUrl.replace(/^https?:\/\//, "")}/${handle}`, company: currentName, defaultValue: "Reserved! {{url}} now belongs to {{company}}." }),
        },
      });
      ...
      dispatch({
        type: TOAST_SHOW,
        payload: {
          message: e?.response?.data?.message || t("handleNudge.reserveFailed", { defaultValue: "Could not reserve the handle. Please try again." }),
          severity: "error",
        },
      });
    } finally {
  ...
  }, [canClaim, dispatch, handle, mutateStorefront, siteUrl, currentName, dismiss, t]);
  ...
  const hintText =
    ...
    || (availability?.available
      ? t("handleNudge.available", { url: `${siteUrl.replace(/^https?:\/\//, "")}/${handle}`, defaultValue: "{{url}} is available — grab it!" })
      : t("handleNudge.pickHint", { company: currentName, defaultValue: "Pick a short, memorable handle for {{company}}." }));
  ...
  return (
    ...
    >
      <IconButton
        ...
        aria-label={t("handleNudge.dismiss", { defaultValue: "Dismiss" })}
        sx={{
          position: "absolute",
          top: 8,
          right: 8,
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.25, pr: 4 }}>
        ...
        <Box sx={{ minWidth: 0 }}>
          ...
          >
            {t("handleNudge.title", { company: currentName, defaultValue: "{{company}} is ready — claim its handle" })}
          </Typography>
          <Typography
            sx={{
              fontSize: 13,
          ...
          >
            <Trans t={t} i18nKey="handleNudge.body" values={{ company: currentName }} components={{ b: <b /> }} defaults="Reserve <b>{{company}}</b>'s public page and checkout URL. It goes live the moment you claim it — no other setup needed." />
          </Typography>
        </Box>
      </Box>
      ...
      <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mt: 0.5 }}>
        ...
        >
          {saving ? (
            ...
            <>{t("handleNudge.claimCta", { handle: handle || "handle", defaultValue: "Claim @{{handle}}" })}</>
          )}
        </Button>
        <Button
          onClick={dismiss}
        ...
        >
          {t("handleNudge.skip", { defaultValue: "Skip for now" })}
        </Button>
      </Box>
    </Box>
  );
