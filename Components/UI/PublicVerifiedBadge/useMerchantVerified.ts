...
export function useMerchantVerified(params: MerchantVerifiedParams): boolean {
  ...
  // The persisted SWR cache (logged-in merchants) can return `true` synchronously on
  // the very first client render; SSR never has it → hold false until hydrated.
  const [hydrated, setHydrated] = useState(false);
  useEffect(() => setHydrated(true), []);
  return hydrated && Boolean(data);
}
