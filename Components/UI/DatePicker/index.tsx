...
import { de as dfDe, es as dfEs, fr as dfFr, nl as dfNl, pt as dfPt } from "date-fns/locale";
import type { Locale } from "date-fns";
import { useTranslation } from "react-i18next";

const DF_LOCALES: Record<string, Locale> = { de: dfDe, es: dfEs, fr: dfFr, nl: dfNl, pt: dfPt };
const dfLocale = (lang: string): Locale | undefined => DF_LOCALES[(lang || "en").split("-")[0]];
import React, {
  forwardRef,
  useCallback,
  useEffect,
...
// Narrow weekday initials in the active locale, Sunday-first (2024-01-07 is a Sunday).
const weekdayInitials = (locale?: Locale) =>
  Array.from({ length: 7 }, (_, i) => ({ key: i, label: format(new Date(2024, 0, 7 + i), "EEEEE", { locale }) }));
    ...
    return (
      ...
      >
        <CalendarHeader>
          ...
          <CalendarHeaderText sx={{ textTransform: "capitalize" }}>{format(month, "MMMM yyyy", { locale })}</CalendarHeaderText>
        ...
        <WeekdayHeader>
          {weekdays.map((day) => (
            <WeekdayCell key={day.key}>{day.label}</WeekdayCell>
          ))}
        </WeekdayHeader>
  ...
  (
    {
      ...
      blockedDateMessage: blockedDateMessageProp,
    ...
    const placeholder = placeholderProp ?? t("datePicker.selectRange", { defaultValue: "Select date range" });
    const blockedDateMessage = blockedDateMessageProp ?? t("datePicker.futureBlocked", { defaultValue: "You can't select a future date." });
    ...
    const formatDateRange = useMemo((): string => {
      ...
      if (startDate && endDate) {
        ...
        return `${format(startDate, "MMM dd, yyyy", { locale })} - ${format(
          endDate,
          "MMM dd, yyyy",
          { locale },
        )}`;
      ...
      if (startDate) {
        ...
        return format(startDate, "MMM dd, yyyy", { locale });
      }
      if (buttonText) {
        return buttonText;
      }
      return placeholder;
    }, [selectedRange, isMobile, buttonText, placeholder, locale]);
          ...
          <StyledDatePickerContainer className={className}>
            <ContentContainer>
              {showPresets && (
                <>
                  <PresetsSidebar>
                    <PresetTitle>{t("datePicker.presets", { defaultValue: "Presets" })}</PresetTitle>
                    {PRESETS.map((preset) => (
                      <PresetItem
                        key={preset.key}
                      ...
                      >
                        <p className="label">{t(`datePicker.preset_${preset.key}`, { defaultValue: preset.label })}</p>
