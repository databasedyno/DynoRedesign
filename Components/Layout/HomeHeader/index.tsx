...
import { SavedMerchantsHeaderLink, SavedMerchantsMobileItem } from "./SavedMerchantsLink";
...
const HomeHeader = memo(function HomeHeader() {
  ...
  // Signed-in merchants browsing public pages get one "Go to dashboard" CTA instead of Log in / Start free.
  ...
  // Hostinger-style header: transparent + tall at the very top, frosted + compact once scrolled.
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        setScrolled(currentY > SCROLLED_PX);
  ...
  // Solid chrome whenever something hangs off the header (menus) or the page is scrolled.
  const solid = scrolled || mobileMenuOpen || openMenu !== null;
      ...
      >
        <MobileDrawer>
          <MobileNavContent>
            ...
            <SavedMerchantsMobileItem onNavigate={() => setMobileMenuOpen(false)} />

            {/* Auth CTAs */}
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.25, mt: 2.5 }}>
              <Button
                data-testid={authed ? "mobile-dashboard-btn" : "mobile-get-started-btn"}
                onClick={() => {
                  setMobileMenuOpen(false);
                  void router.push(authed ? "/dashboard" : "/auth/register");
                }}
