...
const QuickCreateLinkPanel = ({
  ...
  // Quick links should accept EVERY coin the brand has a wallet for, so the
  // buyer sees all payment methods without the merchant picking them one by one.
  // walletData.walletTitle is the wallet_type/currency code (e.g. USDT-TRC20).
  const { walletData } = useWalletData();
  const configuredCurrencies = useMemo(
    () => Array.from(new Set(walletData.map((w) => String(w.walletTitle).trim()).filter(Boolean))),
    [walletData],
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        expire: "No",
        fee_payer: "company",
        // Auto-select all configured coins (empty falls back to "all" server-side).
        accepted_currencies: configuredCurrencies,
        redirect_url: "",
        webhook_url: "",
        callback_url: "",
        apply_tax: false,
      ...
      <Box sx={{ flex: 1, overflowY: "auto", px: 2.5, py: 2.5 }}>
        {!created ? (
          ...
          <Box data-testid="quick-create-success" sx={{ textAlign: "center" }}>
            ...
            <Typography noWrap sx={{ fontFamily: "var(--font-sans)", fontSize: 13.5, color: theme.palette.text.secondary, mb: 1 }}>
              {created.description}
            </Typography>
            <Box sx={{ display: "flex", justifyContent: "center", mb: 2 }}>
              <LinkCoinsBadge value={configuredCurrencies} size="xs" />
            </Box>
            ...
            >
              ...
              <CopyInline
                value={created.url}
                size={17}
                testId="quick-create-copy"
                copyLabel={t("quickCreate.copyLink", { defaultValue: "Copy link" })}
                ...
                onCopied={(ok) => {
                  if (ok) {
                    ...
                    dispatch({
                      type: "TOAST_SHOW",
                      payload: {
                        message: t("quickCreate.copyFailed", { defaultValue: "Copy failed — select and copy manually." }),
                        severity: "error",
                      },
                    });
                  }
