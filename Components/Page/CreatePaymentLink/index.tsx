...
const CreatePaymentLinkPage = ({
  ...
  // < lg: the live preview is an inline collapsible section at the top of the form.
  ...
  const rawWalletList = useWalletStore().walletList;
  const walletList = useMemo(() => rawWalletList ?? [], [rawWalletList]);
  ...
  // E1: /create-pay-link?duplicate=<link_id> prefills the form from an existing
  // link (amount, description, coins, fee payer, URLs) so repeat links take one click.
  const hasAppliedDuplicateRef = useRef(false);
  useEffect(() => {
    if (hasAppliedDuplicateRef.current) return;
    if (hasPaymentLinkData) return;
    if (!router.isReady) return;
    const dupId = String(router.query.duplicate || "").trim();
    if (!dupId) return;
    hasAppliedDuplicateRef.current = true;
    let cancelled = false;
    (async () => {
      try {
        const res = await axiosBaseApi.get(API_ENDPOINTS.pay.link(dupId));
        const d = res?.data?.data;
        if (!d || cancelled) return;
        const coins: string[] = Array.isArray(d.accepted_currencies)
          ? d.accepted_currencies.map((c: string) => String(c).trim().toUpperCase()).filter(Boolean)
          : String(d.accepted_currencies || "").split(",").map((c) => c.trim().toUpperCase()).filter(Boolean);
        if (d.link_type === "donation") setLinkKind("donation");
        setPaymentSettings((prev) => ({
          ...prev,
          value: d.base_amount != null && Number(d.base_amount) > 0 ? String(d.base_amount) : prev.value,
          currency: d.base_currency || prev.currency,
          description: d.description || prev.description,
          blockchainFees: d.fee_payer === "customer" ? "customer" : "company",
          acceptedCryptoCurrency: coins.length ? coins : prev.acceptedCryptoCurrency,
        }));
        setPostPaymentSettings((prev) => ({
          ...prev,
          redirectUrl: d.redirect_url || prev.redirectUrl,
          webhookUrl: d.webhook_url || prev.webhookUrl,
        }));
      } catch {
        /* best-effort prefill — fall back to an empty form */
      }
    })();
    return () => { cancelled = true; };
  }, [router.isReady, router.query.duplicate, hasPaymentLinkData]);
  ...
  const selectedCompany = (companyListForBanner.find((c: any) => c.company_id === selectedCompanyId) || {}) as any;
  const livePreviewProps = {
    ...
    companyName: selectedCompany.company_name || null,
    logoUrl: selectedCompany.photo || null,
  ...
  const previewHasAmount = Number.isFinite(previewAmountNum) && previewAmountNum > 0;
  const previewSummary =
    linkKind === "standard"
      ? `${getCurrencySymbolFromFormat(previewCurrency)}${previewHasAmount ? formatWithSeparators(previewAmountNum, previewCurrency) : "0.00"} ${previewCurrency}`
      : "";
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <Box
            ...
            aria-controls="pay-link-mobile-preview"
          ...
          >
            <Box component="span" sx={{ display: "inline-flex", alignItems: "center", gap: 1, minWidth: 0, whiteSpace: "nowrap" }}>
              ...
              {previewOpen
                ? tPaymentLink("hidePreview", { defaultValue: "Hide customer preview" })
                : tPaymentLink("showPreview", { defaultValue: "Show customer preview" })}
            </Box>
            <Box component="span" sx={{ display: "inline-flex", alignItems: "center", gap: 1, flexShrink: 0 }}>
              {previewSummary && (
                <Typography component="span" noWrap data-testid="mobile-preview-summary" data-has-amount={previewHasAmount ? "true" : "false"} sx={{ fontFamily: MONO, fontVariantNumeric: "tabular-nums", fontSize: 12.5, color: previewHasAmount ? theme.palette.text.secondary : theme.palette.text.disabled, maxWidth: 120 }}>
                  {previewSummary}
                </Typography>
              )}
              <Icon icon={previewOpen ? "mdi:chevron-up" : "mdi:chevron-down"} width={20} />
            </Box>
          </Box>
          {previewOpen && (
            <Box id="pay-link-mobile-preview" sx={{ px: 2, pt: 2, pb: 2.5, borderTop: `1px solid ${theme.palette.border.main}` }}>
              <LivePreviewPanel {...livePreviewProps} compact />
            </Box>
          )}
        </Box>
              ...
              >
                ...
                >
                  <Box component="span" sx={{ display: "inline-flex", alignItems: "center", gap: 1 }}>
                    <Icon icon="mdi:tune-variant" width={16} aria-hidden />
                    {tPaymentLink("moreOptions", { defaultValue: "More options" })}
                    <Typography
                      component="span"
                      sx={{
                        fontFamily: "var(--font-sans)",
            ...
            {feePreview && linkKind === "standard" && (
              ...
              >
                ...
                {paymentSettings.blockchainFees === "customer" && feePreview.customer_pays != null && (
                  <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <Typography sx={{ fontSize: 12, fontFamily: "var(--font-sans)", color: theme.palette.text.secondary }}>
                      {tPaymentLink("feePayerCustomerTitle", { defaultValue: "Customer pays" })}
                    </Typography>
      ...
      <Box
        data-testid="desktop-preview-pane"
        sx={{
          width: 350,
          flexShrink: 0,
          position: "sticky",
      ...
      >
        <LivePreviewPanel {...livePreviewProps} />
      </Box>
      </Box>
    </div>
  );
