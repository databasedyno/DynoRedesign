...
const CryptoTransfer = ({
  ...
  // Smart minimums: per-coin forwarding minimum (USD), this order's USD value,
  // and the cheapest coin's minimum — used to grey out coins the payer can't
  // complete and show each coin's minimum up-front (/pay/configured-currencies).
  const [coinMinimums, setCoinMinimums] = useState<Record<string, number>>({});
  const [orderUsd, setOrderUsd] = useState<number>(0);
  const [minOrderUsd, setMinOrderUsd] = useState<number>(0);
  ...
  useEffect(() => {
    const fetchConfiguredCurrencies = async () => {
      try {
        ...
        // Smart minimums (additive fields; safe if absent on an older backend)
        setCoinMinimums(data?.coin_minimums || {});
        setOrderUsd(Number(data?.transaction_amount_usd || 0));
        setMinOrderUsd(Number(data?.min_order_usd || 0));
  ...
  // ── Smart minimums helpers ────────────────────────────────────────────────
  // Resolve the forwarding minimum (USD) for a coin tile. USDT/RLUSD are shown
  // as one tile but settle per-network → use the cheapest configured network.
  const coinMinFor = (coinValue: string): number => {
    const direct = coinMinimums[coinValue];
    if (typeof direct === 'number') return direct;
    const matches = Object.entries(coinMinimums)
      .filter(([k]) => k === coinValue || k.startsWith(coinValue + '-'))
      .map(([, v]) => Number(v))
      .filter(n => Number.isFinite(n));
    return matches.length ? Math.min(...matches) : 0;
  };
  // A coin is payable when the order's USD value clears its minimum. When the
  // order's USD value is unknown (0 / conversion failed) we NEVER grey a coin —
  // the backend guard still blocks a truly un-forwardable payment.
  const isCoinPayable = (coinValue: string): boolean => {
    if (!orderUsd || orderUsd <= 0) return true;
    return orderUsd >= coinMinFor(coinValue);
  };
  const anyCoinPayable = filteredCryptoOptions.some(o => isCoinPayable(o.value));
  // Network-level minimum (USDT/RLUSD settle per-network). coin_minimums is keyed
  // by the configured currency string (e.g. "USDT-TRC20"; RLUSD-XRPL is "RLUSD").
  const networkMinFor = (base: string, net: string): number => {
    const candidates = [`${base}-${net}`];
    if (base === 'RLUSD' && net === 'XRPL') candidates.push('RLUSD');
    if (base === 'USDC') candidates.push('USDC');
    for (const key of candidates) {
      const v = coinMinimums[key];
      if (typeof v === 'number') return v;
    }
    return coinMinFor(base);
  };
  const isNetworkPayable = (base: string, net: string): boolean => {
    if (!orderUsd || orderUsd <= 0) return true;
  ...
  //     const {
  //       data: { data }
  //     } = await axiosBaseApi.post(API_ENDPOINTS.pay.verifyCryptoPayment, {
  //       address: cryptoDetails?.address
  //     })
      ...
      >
        ...
        <FormControl fullWidth>
          {loadingCurrencies ? (
            ...
            <>
            {orderUsd > 0 && !anyCoinPayable && (
              <Box
                data-testid="crypto-below-min-banner"
                mb={1.5}
                py={1.25}
                px={2}
                border="1px solid #f59e0b"
                ...
                bgcolor={isDark ? 'rgba(255, 251, 235, 0.1)' : "#fffbeb"}
              >
                <Typography fontFamily="var(--font-sans)" color="#b45309" fontSize="13px" fontWeight={600}>
                  {t('crypto.belowAllMinimums', { min: minOrderUsd, defaultValue: `This amount is below the minimum for every coin. Minimum is $${minOrderUsd}. Increase the amount to continue.` })}
                </Typography>
              </Box>
            )}
            ...
            >
              {filteredCryptoOptions?.map((option) => {
                ...
                const payable = isCoinPayable(option.value);
                const coinMin = coinMinFor(option.value);
                const coinLabel = option.value === "POLYGON" ? "POL" : option.value;
                return (
                  <Box
                    key={option.value}
                    component="button"
                    ...
                    title={!payable ? t('crypto.coinMinTooltip', { min: coinMin, coin: coinLabel, defaultValue: `Minimum $${coinMin} to pay with ${coinLabel}` }) : undefined}
                    onClick={() => { if (!payable) return; handleChange({ target: { value: option.value } }); }}
                    ...
                    sx={{
                      ...
                      "&:hover": {
                        borderColor: !payable ? theme.palette.border.main : (isSelected ? ACCENT : theme.palette.text.disabled),
                      },
                    }}
                  >
                    ...
                    {!payable && coinMin > 0 && (
                      <Typography
                        data-testid={`crypto-min-${option.value}`}
                        fontSize="10px"
                        fontWeight={600}
                        fontFamily="var(--font-sans)"
                        color="#b45309"
                        ...
                        mt={0.25}
                      >
                        {t('crypto.coinMinBadge', { min: coinMin, defaultValue: `Min $${coinMin}` })}
                      </Typography>
                    )}
                  </Box>
                );
          ...
          availableUSDTNetworks.length > 0 ? (
            <Box mt={1} mb={2.5} display="flex" gap={1} alignItems="center" flexWrap="wrap">
              {availableUSDTNetworks.map((net) => {
                const netPayable = isNetworkPayable('USDT', net);
                const netMin = networkMinFor('USDT', net);
                return (
                <Typography
                  key={net}
                  component="button"
                  ...
                  title={!netPayable ? `Minimum $${netMin} to pay with USDT-${net}` : undefined}
                  ...
                  color={theme.palette.text.primary}
                  borderRadius="10px"
                  sx={{ cursor: netPayable ? "pointer" : "not-allowed", opacity: netPayable ? 1 : 0.45, transition: "border-color 0.15s ease, background-color 0.15s ease", minHeight: { xs: 44, md: "auto" } }}
                  onClick={() => { if (!netPayable) return; handleNetworkChange(net); }}
                  fontFamily="var(--font-sans)"
                >
                  {net}{!netPayable ? ` · min $${netMin}` : ''}
                </Typography>
                );
              })}
          ...
          availableRLUSDNetworks.length > 0 ? (
            <Box mt={1} mb={2.5} display="flex" gap={1} alignItems="center" flexWrap="wrap">
              {availableRLUSDNetworks.map((net) => {
                const netPayable = isNetworkPayable('RLUSD', net);
                const netMin = networkMinFor('RLUSD', net);
                return (
                <Typography
                  key={net}
                  component="button"
                  disabled={!netPayable}
                  data-testid={`network-tile-${net}`}
                  title={!netPayable ? `Minimum $${netMin} to pay with RLUSD-${net}` : undefined}
                  border={`1.5px solid ${selectedNetwork === net ? ACCENT : theme.palette.border.main}`}
                  padding="7px 16px"
                  fontSize="13px"
                  fontWeight={600}
                  ...
                  borderRadius="10px"
                  sx={{ cursor: netPayable ? "pointer" : "not-allowed", opacity: netPayable ? 1 : 0.45, transition: "border-color 0.15s ease, background-color 0.15s ease", minHeight: { xs: 44, md: "auto" } }}
                  onClick={() => { if (!netPayable) return; handleRLUSDNetworkChange(net); }}
                  fontFamily="var(--font-sans)"
                >
                  {net}{!netPayable ? ` · min $${netMin}` : ''}
                </Typography>
                ...
                >
                  ...
                  {!isPartialPaymentMode && (
                    ...
                    <Box mb={2} data-testid="fee-breakdown">
                      ...
                      {/* Processing fee row - only when the buyer pays it */}
                      {feeInfo && feeInfo.fee_payer === 'customer' && ((feeInfo.processing_fee ?? 0) > 0 || (selectedCurrency?.processing_fee ?? 0) > 0) && (
                        <Box display="flex" justifyContent="space-between" mb={0.5}>
                          <Typography
                            variant="body2"
