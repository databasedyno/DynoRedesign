...
const ProductEditor: React.FC<ProductEditorProps> = ({ mode, productId }) => {
  ...
  // Shared by the file <input> and the drag-and-drop target.
  ...
  const handleVariantFile = (file: File, idx: number) => {
    if (isCroppableImage(file.type)) {
  ...
  const onVariantImgChosen = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && idx != null) handleVariantFile(file, idx);
  };
    ...
    <Stack spacing={2} data-testid="product-editor">
      ...
      <PanelCard title={mode === "new" ? t("productEditor.newProduct", { defaultValue: "New product" }) : t("productEditor.editProduct", { defaultValue: "Edit product" })}>
        <Stack spacing={2}>
          ...
          <ImageDropTarget onFile={handleGalleryFile} disabled={galleryUploading} radius={10} testId="product-gallery-dropzone">
        ...
        <Stack spacing={2}>
          ...
          <FormControl fullWidth>
            ...
            >
              ...
              <MenuItem value="physical" data-testid="product-tax-category-opt-physical">{t("productEditor.categoryPhysical", { defaultValue: "Physical goods (no shipping address collected)" })}</MenuItem>
            ...
            <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5 }}>
              {taxCategory === "exempt"
                ...
                ? t("productEditor.categoryPhysicalHint", { defaultValue: "Heads-up: the store checkout is built for digital goods and services — it doesn't collect a shipping address. Tax uses the buyer's detected location; arrange delivery details with the buyer by email." })
      ...
      >
        <Stack spacing={1}>
          ...
          {hasVariants && variants.map((v, idx) => (
            ...
            >
              {/* Variant image thumbnail + upload (drop target) */}
              <ImageDropTarget onFile={(f) => handleVariantFile(f, idx)} disabled={variantImgUploadIdx !== null} radius={8} testId={`product-variant-dropzone-${idx}`}>
