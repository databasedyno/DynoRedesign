...
const ShareTab: React.FC = () => {
  ...
  const shareText = profile?.name
    ? t("storefront.share.textNamed", { name: profile.name, defaultValue: "Pay {{name}} in crypto — no signup needed." })
    : t("storefront.share.textMe", { defaultValue: "Pay me in crypto — no signup needed." });
  const targets = [
    {
      key: "x",
      icon: "mdi:twitter",
    ...
    },
    {
      ...
      label: t("storefront.share.email", { defaultValue: "Email" }),
      href: `mailto:?subject=${encodeURIComponent(t("storefront.share.emailSubject", { defaultValue: "My Dynopay page" }))}&body=${encodeURIComponent(`${shareText} ${url}`)}`,
    },
  ];
      ...
      <PanelCard title="">
        ...
        >
          ...
          <Typography sx={{ fontSize: 17, fontWeight: 700, mt: 1.25 }}>
            {t("storefront.share.claimTitle", { defaultValue: "Claim your handle to get a link" })}
          </Typography>
          <Typography
            sx={{
              fontSize: 13.5,
          ...
          >
            {t("storefront.share.claimBody", { defaultValue: "Your page lives at a short URL you can put in any bio. Pick a handle on the Page tab and this is where you'll share it." })}
          </Typography>
          <CustomButton
            label={t("storefront.share.setupCta", { defaultValue: "Set up my page" })}
            variant="primary"
            size="medium"
  ...
  return (
    ...
    >
      <PanelCard
        title={t("storefront.share.linkTitle", { defaultValue: "Your page link" })}
        subTitle={
          isPublished
            ? t("storefront.share.liveSub", { defaultValue: "Live — anyone with this link can tip you, buy your products and pay your links." })
            : t("storefront.share.unpublishedSub", { defaultValue: "Not published yet — turn on the publish toggle on the Page tab to make this link work." })
        }
      >
        <Box
          sx={{
        ...
        >
          ...
          >
            ...
            <CopyInline
              value={url}
              size={18}
              testId="storefront-share-copy"
              copyLabel={t("storefront.share.copyLink", { defaultValue: "Copy link" })}
              copiedLabel={t("storefront.share.linkCopied", { defaultValue: "Link copied" })}
            />
          </Box>
          <CustomButton
            label={t("storefront.share.open", { defaultValue: "Open" })}
            variant="outlined"
            size="small"
      ...
      <PanelCard
        title={t("storefront.share.qrTitle", { defaultValue: "QR code" })}
        subTitle={t("storefront.share.qrSub", { defaultValue: "Print it, drop it in a video, stick it on a flyer — it opens your page." })}
      >
