    ...
    <PanelCard title="">
      <Box data-testid="storefront-pending" sx={{ textAlign: "center", py: 6, px: 3 }}>
        ...
        <Typography sx={{ fontSize: 17, fontWeight: 700, mt: 1.25 }}>
          {currentName} doesn&apos;t have its own storefront yet
        </Typography>
        <Typography
          sx={{
            fontSize: 13.5,
        ...
        >
          Your account currently has one shared storefront
          {accountHandle ? (
            <>
              {" "}(
          ...
          and it belongs to {primaryName}. Separate storefronts for each company
          are coming soon — switch to {primaryName} to edit the live page.
        </Typography>
        {Number.isFinite(primaryId) && primary ? (
          <CustomButton
            label={`Switch to ${primaryName}`}
            variant="primary"
            size="medium"
