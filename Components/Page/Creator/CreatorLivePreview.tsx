...
const CreatorLivePreview: React.FC<Props> = ({ state }) => {
  ...
  const name = (draftName && draftName.trim()) || selectedCompany?.company_name || profile?.name || state.handle || "Your name";
  const handle = state.handle || "yourname";
  const bio = state.bio || null;
  // The public page shows the BRAND logo (per-brand storefront) — never the account photo.
  const photo = selectedCompany?.photo && !String(selectedCompany.photo).includes("user_image.png") ? selectedCompany.photo : null;
      ...
      <Box sx={{ px: 3, pt: 0, pb: 3, mt: -5, textAlign: "center", position: "relative", zIndex: 1 }}>
        ...
        >
          ...
          <Typography fontWeight={700} fontSize={14} color={theme.palette.text.primary} mt={0.25}>
            {t("storefront.preview.firstDonationTitle", { defaultValue: "Your first donation link will appear here" })}
          </Typography>
          <Typography fontSize={11.5} color={theme.palette.text.secondary} mt={0.5}>
            {t("storefront.preview.firstDonationBody", { defaultValue: "Create a donation link and it will feature at the top of your page." })}
          </Typography>
          <LinearProgress
            variant="determinate"
            value={35}
