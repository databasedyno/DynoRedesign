...
interface Meta {
  ...
  merchant?: { name?: string; company_name?: string | null; company_logo?: string | null; handle?: string | null } | null
}
...
// i18n key suffixes per mode — resolved through `t()` inside the component so
// every public string on the creator/store checkout is translatable (C7).
const MODE_DEFAULTS: Record<InlineMode, {
  preparing: string
  ...
  tip: {
    ...
    successSubject: 'You {{verb}} {{target}}',
    shareText: 'I just {{verb}} {{target}}! {{url}}',
  ...
  link: {
    ...
    successSubject: 'Paid to {{target}}',
    shareText: 'Just paid {{target}} on Dynopay. {{url}}',
    backLabel: 'Back to {{target}}',
  ...
  donation: {
    ...
    successTitle: 'Thank you{{name}}!',
    successSubject: 'You supported {{target}}',
    shareText: 'I just supported {{target}}. {{url}}',
    backLabel: 'Back to {{target}}',
...
interface InlineTipCheckoutProps {
  ...
  /** D2: where the buyer's purchase lives after paying (e.g. /order/<ref>).
   *  Rendered as the PRIMARY success action and auto-followed after a short
   *  countdown so downloads / license keys are never one email away. */
  successHref?: string
  successLabel?: string
}
...
const InlineTipCheckout: React.FC<InlineTipCheckoutProps> = ({
  ...
  const mc = (key: keyof typeof md, vars: Record<string, unknown> = {}) =>
    t(`creator.inline.mode.${mode}.${key}`, { defaultValue: md[key], ...vars })
  const successVerb = t(`creator.inline.verb.${style}`, { defaultValue: meta.successVerb })
  ...
  // True once the chain has SEEN the payment (backend 'pending'/'processing').
  // Drives the Detected step and stops the countdown from expiring (B1).
  ...
  // Fiat total actually charged (base + tax + buyer-paid fee) — the "≈ $" line.
  const [totalFiat, setTotalFiat] = useState<number | null>(null)
  const [timeLeft, setTimeLeft] = useState<number>(0) // seconds
  const [totalSeconds, setTotalSeconds] = useState<number>(0) // reservation window (for the countdown bar)
  const [copiedFlag, setCopiedFlag] = useState<string>('')
  // Success-screen proof of payment (same endpoints as the hosted checkout).
  ...
  // C2 parity: the QR is visible by default — scanning is the #1 mobile path.
  const [qrExpanded, setQrExpanded] = useState<boolean>(true)
  ...
  const metaSwr = useSWR(
    ...
    async () => api('/pay/getData', { data: d, language: i18n.language || 'en' }, undefined),
  ...
  const pickCurrency = useCallback(
    async (code: string) => {
      ...
      // 1. Fetch the crypto rate to know the expected crypto amount. Forward
      //    fee_payer + tax exactly like CleanCheckoutV2 so customer-pays links
      //    charge the fee-inclusive total (otherwise the merchant eats the fee).
      ...
      const rateRes = await api(
        '/pay/getCurrencyRates',
        {
          ...
          amount: feePayer === 'customer' ? baseAmount : baseAmount + taxAmount,
      ...
      if (feePayer === 'customer' && (rateRow.fee_error || rateRow.total_amount == null)) {
        setErrorMsg(t('checkout.feeCalcError', { defaultValue: 'Could not calculate the network fee for this coin. Please try again or choose a different coin.' }))
        setPhase('error')
        return
      }
      ...
      const exactTotal = feePayer === 'customer'
        ? Number(rateRow.total_amount_source ?? rateRow.total_amount_usd) || (baseAmount + taxAmount + (Number(rateRow.processing_fee) || 0))
        : baseAmount + taxAmount
      setTotalFiat(exactTotal)
  ...
  useEffect(() => {
    ...
    // Fire once per phase transition only
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase])
  ...
  const applyVerifyResult = useCallback((r: Awaited<ReturnType<typeof api>>) => {
    ...
    if (s === 'pending' || s === 'processing') {
      // Chain has seen the tx — show the Detected step and stop the countdown
      // from expiring (the network confirms regardless of the window).
      setDetected(true)
      return
    }
    if (s === 'underpaid') {
  ...
  // ─── Countdown timer ──────────────────────────────────────────────────
  // B1: never expire a checkout whose payment is already on-chain — hold at
  // 0:00 and keep polling until the backend reports the final state.
  const timerArmed = timeLeft > 0
  useEffect(() => {
    if (phase !== 'awaiting_payment' && phase !== 'underpaid') return
  ...
  // D2: auto-follow the post-payment destination (order page) after a short
  // countdown; the buyer can cancel and stay.
  const goToSuccessHref = useCallback(() => {
    if (!successHref || typeof window === 'undefined') return
    window.location.assign(successHref)
  }, [successHref])
  useEffect(() => {
    if (phase !== 'confirmed' || !successHref) return
    setRedirectIn(8)
    const id = setInterval(() => {
      setRedirectIn((n) => {
        if (n == null) return n
        if (n <= 1) { clearInterval(id); goToSuccessHref(); return 0 }
        return n - 1
      })
    }, 1000)
  ...
  const handleDownloadReceipt = async () => {
    if (receiptState === 'busy' || !cryptoInfo?.address || !meta_?.token) return
  ...
  const handleCopyReceiptLink = async () => {
    if (receiptLinkState === 'busy' || !cryptoInfo?.address || !meta_?.token) return
    setReceiptLinkState('busy')
    try {
      const { url } = await fetchReceiptLink(cryptoInfo.address, meta_.token)
      const ok = await copyToClipboard(url)
      if (ok === false) window.open(url, '_blank', 'noopener')
    ...
    return (
      ...
      >
        ...
        <Typography fontSize={13} color={theme.palette.text.secondary} mt={0.5}>
          {errorMsg || t('checkout.pleaseTryAgain', { defaultValue: 'Please try again.' })}
        </Typography>
        <Button
          fullWidth
          variant="outlined"
  ...
  if (phase === 'awaiting_payment' && cryptoInfo && meta_) {
    ...
    const shared = SHARED_INFO[cryptoInfo.crypto_display]
    const networkLabel = shared?.networkLabel || info?.label || cryptoInfo.crypto_display
    const uri = buildPaymentUri(shared?.network || cryptoInfo.network, cryptoInfo.address, cryptoInfo.expected_amount, cryptoInfo.crypto_base)
      ...
      <Box>
        ...
        <Typography fontSize={12} color={theme.palette.text.secondary} mb={1.5} data-testid="inline-tip-fiat-line">
          ...
          {feePayerIsCustomer && totalFiat != null && totalFiat > meta_.amount
            ...
            : ''}
          {' — '}{t('creator.inline.onNetwork', { network: networkLabel, defaultValue: `on ${networkLabel}` })}
        </Typography>
        ...
        {/* QR — visible by default (C2). URI-encoded for native coins so a
              ...
              >
                {uri ? (
                  ...
                  <Box
                    ...
                    sx={{ display: 'block', width: 176, height: 176 }}
                  />
                )}
              </Box>
            )}
        ...
        {/* One-tap wallet deep link (native coins only — BIP-21 / Solana Pay) */}
        {uri && (
          <Box
            component="a"
            href={uri.uri}
        ...
        {/* Wrong-coin / wrong-network warning — the #1 way buyers lose funds */}
        <Box
          data-testid="inline-tip-network-warning"
          sx={{ mt: 1.5, display: 'flex', alignItems: 'flex-start', gap: 0.9, p: 1.25, borderRadius: '10px', backgroundColor: warnTint, border: '1px solid rgba(180,83,9,0.25)' }}
        >
          <Icon icon="mdi:alert-outline" width={17} color="#B45309" style={{ flexShrink: 0, marginTop: 1 }} />
          <Typography sx={{ fontSize: 12, lineHeight: 1.5, color: theme.palette.text.primary }}>
            {t('creator.inline.networkWarning', {
              coin: cryptoInfo.crypto_base,
              network: networkLabel,
              defaultValue: `Send only ${cryptoInfo.crypto_base} on the ${networkLabel} network. Other coins or networks can't be recovered.`,
            })}
          </Typography>
        </Box>
  ...
  if (phase === 'underpaid' && cryptoInfo && partial && meta_) {
    ...
    return (
      ...
      >
        ...
        <Typography fontSize={12.5} color={theme.palette.text.secondary} mt={0.5}>
          {t('checkout.underpaid.gotPrefix', { defaultValue: 'We received' })} {formatCryptoAmount(partial.paidAmount, cryptoInfo.crypto_base)} {cryptoInfo.crypto_base}. {t('checkout.underpaid.sendPrefix', { defaultValue: 'Send' })}{' '}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1.5 }}>
          <Typography fontSize={11.5} color={theme.palette.text.secondary}>
            <Icon icon="mdi:timer-outline" width={12} style={{ verticalAlign: 'middle' }} /> {t('creator.inline.gracePeriod', { minutes: partial.graceMinutes || 30, defaultValue: `${partial.graceMinutes || 30}-min grace period` })}
          </Typography>
  ...
  if (phase === 'confirmed' && confirmedAmount && meta_) {
    ...
    const fiatStr = `${sym}${formatWithSeparators(confirmedAmount.fiat || meta_.amount, confirmedAmount.fiatCurrency || meta_.base_currency)}`
    const shareText = mc('shareText', { target: effectiveTarget, verb: successVerb, url: shareUrl })
    const showDonorMsg = !isAnon && donorMessage && donorMessage.trim().length > 0
    const thanksName = !isAnon && donorName ? `, ${donorName}` : ''
    return (
      <Box data-testid="inline-tip-success">
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          >
            {successHref
              ? t('creator.inline.successNextOrder', { defaultValue: 'Your payment is confirmed. Your downloads, license keys and booking details are on your order page.' })
        ...
        {/* D2: the purchase lives on the order page — make it the primary action */}
        {successHref && (
          <Box sx={{ mt: 2 }}>
            <Button
              fullWidth
              variant="contained"
              disableElevation
              data-testid="inline-tip-success-order-btn"
              onClick={goToSuccessHref}
            ...
            >
              {successLabel || t('creator.inline.viewOrder', { defaultValue: 'View your order & downloads' })}
            </Button>
            {redirectIn != null && redirectIn > 0 && (
              <Typography data-testid="inline-tip-redirect-countdown" sx={{ fontSize: 12, color: theme.palette.text.secondary, mt: 0.75, textAlign: 'center' }}>
                {t('checkout.redirectingIn', { defaultValue: 'Taking you there in {{s}}s', s: redirectIn })}{' '}
                <Box component="button" type="button" onClick={() => setRedirectIn(null)} data-testid="inline-tip-redirect-cancel" sx={{ background: 'none', border: 'none', p: 0, cursor: 'pointer', color: theme.palette.text.primary, fontSize: 12, fontWeight: 700, textDecoration: 'underline' }}>
        ...
        {/* Proof of payment — PDF receipt + shareable receipt link (parity with the hosted checkout) */}
        <Box data-testid="inline-tip-receipt-row" sx={{ mt: 1.5, display: 'flex', gap: 1, flexWrap: 'wrap', justifyContent: 'center' }}>
          <Button
            variant="outlined"
            disableElevation
            data-testid="inline-tip-receipt-btn"
            onClick={handleDownloadReceipt}
            disabled={receiptState === 'busy'}
            startIcon={<Icon icon={receiptState === 'busy' ? 'mdi:loading' : receiptState === 'done' ? 'mdi:check' : 'mdi:file-download-outline'} width={17} />}
            sx={{ textTransform: 'none', borderRadius: '999px', fontWeight: 700, fontSize: 13, px: 2, minHeight: 38, color: theme.palette.text.primary, borderColor: border, '&:hover': { borderColor: theme.palette.text.primary, backgroundColor: 'transparent' } }}
          >
            {receiptState === 'busy'
          ...
          <Button
            variant="outlined"
            disableElevation
            data-testid="inline-tip-receipt-link-btn"
            onClick={handleCopyReceiptLink}
            disabled={receiptLinkState === 'busy'}
            startIcon={<Icon icon={receiptLinkState === 'busy' ? 'mdi:loading' : receiptLinkState === 'copied' ? 'mdi:check' : 'mdi:link-variant'} width={17} color={receiptLinkState === 'copied' ? LIME : undefined} />}
            sx={{ textTransform: 'none', borderRadius: '999px', fontWeight: 700, fontSize: 13, px: 2, minHeight: 38, color: theme.palette.text.primary, borderColor: receiptLinkState === 'copied' ? LIME : border, '&:hover': { borderColor: theme.palette.text.primary, backgroundColor: 'transparent' } }}
          >
        ...
        {(receiptState === 'error' || receiptLinkState === 'error') && (
          <Typography data-testid="inline-tip-receipt-error" sx={{ fontSize: 12, color: '#EF4444', mt: 0.75, textAlign: 'center' }}>
            {receiptState === 'error'
        ...
        {/* Keep this merchant for next time — device-local, no account */}
        <SaveMerchantButton handle={meta_.merchant?.handle || handle} name={meta_.merchant?.name || meta_.merchant?.company_name || creatorName || `@${handle}`} avatar={meta_.merchant?.company_logo} accent={LIME} />
        ...
        {meta_?.contribution?.show_supporters && (meta_?.contribution?.supporters_count || meta_?.contribution?.raised_amount) ? (
          ...
          >
            {meta_.contribution?.supporters_count || 0} {t('creator.supporters', { defaultValue: 'supporters' })} · {sym}
            {formatWithSeparators(Number(meta_.contribution?.raised_amount || 0), meta_.contribution?.campaign_currency || meta_.base_currency)} {t('creator.raised', { defaultValue: 'raised' })}
          </Typography>
        ) : null}
        ...
        <Box sx={{ mt: 2, display: 'flex', gap: 0.75, flexWrap: 'wrap', justifyContent: 'center' }}>
          ...
          >
            <Icon icon="mdi:twitter" width={14} /> {t('creator.shareLabel', { defaultValue: 'Share' })}
          </Box>
          <Box
            component="a"
        ...
        <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
          ...
          <Button
            fullWidth
            variant={successHref ? 'outlined' : 'contained'}
            disableElevation
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
          ...
          <Typography fontWeight={800} fontSize={15}>
            {isExpired
              ...
              : t('checkout.failedTitle', { defaultValue: 'Payment failed' })}
          </Typography>
        </Box>
        <Typography fontSize={13} color={theme.palette.text.secondary} mt={0.5}>
          {isExpired
            ? t('creator.inline.expiredBody', { defaultValue: 'No payment was received in time. Nothing was charged — you can try again.' })
            : t('creator.inline.failedBody', { defaultValue: "We couldn't process your payment. Please try again." })}
        </Typography>
        {isExpired && selectedCurrency && (
          <Button
            ...
            variant="contained"
            disableElevation
            onClick={() => pickCurrency(selectedCurrency)}
        ...
        <Box sx={{ display: 'flex', gap: 1, mt: isExpired && selectedCurrency ? 1 : 2 }}>
          <Button
            fullWidth
            variant="outlined"
            onClick={onCancel}
          ...
          >
            {t('creator.inline.cancel', { defaultValue: 'Cancel' })}
          </Button>
          <Button
            fullWidth
            variant={isExpired && selectedCurrency ? 'outlined' : 'contained'}
            disableElevation
            onClick={onNewTip}
            data-testid="inline-tip-retry-btn"
            sx={isExpired && selectedCurrency
