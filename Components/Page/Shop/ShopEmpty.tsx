...
export default function ShopEmpty({ merchant, isOwner }: Props) {
  ...
  // D7: visitors get a visitor-appropriate empty state — never merchant
  // onboarding CTAs ("Add your first product") on a public page.
  if (!isOwner) {
    return (
      <Box
        data-testid="shop-empty"
        sx={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", py: { xs: 5, md: 8 }, px: 2, color: isDark ? "rgba(255,255,255,0.85)" : "rgba(0,0,0,0.75)" }}
      >
        <Box sx={{ color: isDark ? "rgba(255,255,255,0.9)" : "rgba(0,0,0,0.75)", mb: 2 }}>{ILLUSTRATION}</Box>
        <Typography component="h2" sx={{ fontWeight: 800, fontSize: { xs: "1.5rem", md: "1.85rem" }, lineHeight: 1.2, letterSpacing: "-0.02em", fontFamily: "var(--font-hero), var(--font-sans)", mb: 1 }}>
          {t("shop.emptyVisitorTitleV2", { name: merchant.name, defaultValue: `${merchant.name} hasn't listed anything yet` })}
        </Typography>
        <Typography variant="body1" sx={{ maxWidth: 520, color: isDark ? "rgba(255,255,255,0.65)" : "rgba(0,0,0,0.6)", mb: 3, lineHeight: 1.55 }}>
          {t("shop.emptyVisitorBodyV2", { defaultValue: "Check back soon — or show your support on their page in the meantime." })}
        </Typography>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5}>
          <Button
            ...
            variant="contained"
            disableElevation
            data-testid="shop-empty-visit-page"
            sx={{ fontWeight: 700, borderRadius: 999, textTransform: "none", px: 3 }}
          >
            {t("shop.emptyVisitPage", { name: merchant.name, defaultValue: `Visit ${merchant.name}'s page` })}
          </Button>
        </Stack>
      </Box>
    );
