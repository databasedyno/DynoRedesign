    ...
    return (
      ...
      >
        ...
        <Dialog open={qrOpen} onClose={() => setQrOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: "16px" } }}>
          ...
          <DialogActions>
            <Button onClick={() => setQrOpen(false)} sx={{ textTransform: "none" }}>{t("close", { ns: "common", defaultValue: "Close" })}</Button>
          </DialogActions>
        </Dialog>
        ...
        <Dialog open={analyticsOpen} onClose={() => setAnalyticsOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: "16px" } }} data-testid="creator-analytics-dialog">
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <Box>
                <Typography sx={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: theme.palette.text.secondary, mb: 0.75 }}>
                  {t("creatorCardReferrers", { defaultValue: "Referrers ({{count}})", count: topReferrers.length })}
                </Typography>
                {topReferrers.length === 0 && (
                  <Typography sx={{ fontSize: 13, color: theme.palette.text.secondary }}>
                    {t("creatorCardNoReferrers", { defaultValue: "No referrer data yet — visits will start showing up here as your link is shared." })}
                  </Typography>
                )}
                {topReferrers.map((r) => (
          ...
          <DialogActions>
            <Button onClick={() => setAnalyticsOpen(false)} sx={{ textTransform: "none" }}>{t("close", { ns: "common", defaultValue: "Close" })}</Button>
          </DialogActions>
        </Dialog>
      </PanelCard>
    );
