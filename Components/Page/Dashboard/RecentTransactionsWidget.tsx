...
import { toTxStatusBucket, TX_STATUS_TONE } from "@/helpers/txStatus";
...
const isInternalCustomerEmail = (email: string): boolean => {
  ...
  return (
    lower.endsWith(".local") || // @dynopay.local AND @<brand-slug>.local (e.g. buyer@nameword.local)
    lower.endsWith("@dynopay.internal") ||
    lower.startsWith("legacy-api-") ||
    lower.startsWith("pk-buyer-") ||
    lower.startsWith("elements-buyer-") ||
    lower.startsWith("recovered-")
  );
};
...
const statusStyle = (status: string, theme: any) => {
  ...
  const { fg, dot } = statusToneColors(TX_STATUS_TONE[bucket], isDark);
  const icon =
    bucket === "settled" || bucket === "confirmed" ? (
      <CheckCircleRounded sx={{ fontSize: 14 }} />
    ) : bucket === "failed" ? (
      <ErrorOutlineRounded sx={{ fontSize: 14 }} />
    ) : (
      <HourglassEmptyRounded sx={{ fontSize: 14 }} />
    );
  return { color: fg, bg: `${dot}${isDark ? "29" : "1A"}`, icon };
};
