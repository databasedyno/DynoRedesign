  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        >
          {t("reuseNudge.body", {
            count: source.wallet_count,
            coins: source.wallets.slice(0, 4).map((w) => w.currency).join(", ") + (source.wallet_count > 4 ? "…" : ""),
            target: targetName,
            defaultValue: "{{count}} address(es) ({{coins}}) are already saved on your other account — copy them to {{target}} and start accepting payments now.",
          })}
        </Typography>
      </Box>
      ...
      >
        ...
        {onAddWallet && (
          ...
          >
            {t("reuseNudge.addDifferent", { defaultValue: "Add a different one" })}
          </Box>
        )}
      </Box>
    </Box>
