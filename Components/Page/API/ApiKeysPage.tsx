...
const ApiKeyCard = ({ title, apiRow, revealedKey, onCopy, onDelete, onRegenerate, onToggleStatus }: ApiKeyCardProps & { onRegenerate?: (id: string | number) => void; onToggleStatus?: (id: string | number, status: string) => void }) => {
  const { t } = useTranslation("apiScreen");
  ...
  // Hash-only storage: the full key exists client-side ONLY right after
  // create/regenerate (revealedKey). Otherwise all we have is the hint.
  const apiKey: string = revealedKey || "";
  const keyHint: string = apiRow?.key_hint || apiRow?.apiKey_masked || "";
  const hasRevealedKey = apiKey.length > 0;
  ...
  const handleCurrencyChange = async (next: string) => {
    ...
    try {
      ...
      dispatch({
        type: TOAST_SHOW,
        payload: { message: t("currency.updated", { currency: upper, defaultValue: "Settlement currency updated to {{currency}}" }), severity: "success" },
      });
  ...
  const displayApiKey = hasRevealedKey
    ? showApiKey
      ? apiKey
  ...
  return (
    <PanelCard
      ...
      headerSx={{ flexWrap: "wrap", rowGap: 1 }}
      headerAction={
        <Box
          sx={{
            display: "flex",
            ...
            // Static in the header row so the badges never overlap the title;
            // on phones the header wraps and the badges drop under it.
            position: "static",
            zIndex: 1,
    ...
    >
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          ...
          <CustomButton
            ...
            label={t("currency.confirmCta", { currency: pendingCurrency || "", defaultValue: "Change to {{currency}}" })}
      ...
      <ApiKeyCardBody sx={{ pt: isMobile ? "16px" : "18px" }}>
          ...
          {hasRevealedKey && (
            <>
              <CopyInline
                ...
                size={16}
                testId="api-key-copy"
                copyLabel={t("icons.copyAlt")}
                sx={{ [theme.breakpoints.down("md")]: { width: 32, height: 32 } }}
              />
              <ApiKeyViewButton
                size="small"
        ...
        <Box
          data-testid={hasRevealedKey ? "api-key-copy-now-notice" : "api-key-shown-once-notice"}
          sx={{
            mt: 1,
            display: "flex",
            ...
            lineHeight: 1.5,
            fontFamily: "var(--font-sans)",
            color: hasRevealedKey
              ? theme.palette.mode === "dark" ? "#FCD34D" : "#92400E"
              : theme.palette.text.secondary,
            background: hasRevealedKey
              ? theme.palette.mode === "dark" ? "rgba(251,191,36,0.10)" : "#FFFBEB"
              : theme.palette.mode === "dark" ? "rgba(255,255,255,0.04)" : "#F8FAFC",
            border: `1px solid ${
              hasRevealedKey
                ? theme.palette.mode === "dark" ? "rgba(251,191,36,0.35)" : "#FDE68A"
                : theme.palette.border.main
            }`,
          }}
        >
          <Icon name={hasRevealedKey ? "triangle-alert" : "lock"} size={15} style={{ flexShrink: 0, marginTop: 2 }} />
          <span>
            {hasRevealedKey
              ? t("keys.copyNowNotice", {
                  defaultValue: "Copy this key now — for security it will not be shown again once you leave this page.",
                })
              : t("keys.shownOnceNotice", {
                  defaultValue: "Keys are shown once. Only a one-way hash is stored, so this key can't be revealed — press Regenerate to get a new one you can copy.",
                })}
          </span>
        </Box>
        {isSandboxKey && hasRevealedKey && (
          <Box sx={{ mt: 1.25 }}>
            <Box
              component="button"
              type="button"
    ...
    >
      ...
      <Box
        sx={{
          ...
          borderRadius: "10px",
          border: `1px solid ${theme.palette.border.main}`,
          background: theme.palette.mode === "dark" ? "rgba(129,140,248,0.08)" : "#f5f7ff",
        }}
      >
          ...
          apiState.apiList.map((api: IApi, index: number) => (
            ...
            >
              <ApiKeyCard
                ...
                apiRow={api}
                revealedKey={apiState?.revealedKeys?.[api.api_id]}
                onCopy={handleCopy}
                onDelete={requestDelete}
