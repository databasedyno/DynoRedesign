...
const WebhookConsoleSection = ({ view = "all" }: { view?: "all" | "settings" | "events" }) => {
  ...
  const resendDelivery = async (logId: number) => {
    if (!companyId || resendingId) return;
    setResendingId(logId);
    try {
      const res = await axiosBaseApi.post(API_ENDPOINTS.company.webhookHistoryResend(companyId, logId));
      const ok = res?.data?.data?.resent;
      if (ok) {
        toast(tr("webhook.toastResent", { defaultValue: "Event re-sent" }));
      } else {
        const err = res?.data?.data?.error ? `: ${res.data.data.error}` : "";
        toast(tr("webhook.toastResendFailed", { defaultValue: "Re-send attempted but the endpoint did not accept it{{err}}", err }), "error");
      }
      refreshAll();
    } catch {
      toast(tr("webhook.toastResendError", { defaultValue: "Failed to re-send this event" }), "error");
    } finally {
      setResendingId(null);
    }
  };
            ...
            <Box sx={{ p: 2.5 }}>
              ...
              <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 2 }}>
                <Button
                  variant="contained"
                  size="small"
                  disabled={!detail.webhook_url || resendingId === detail.log_id}
                  onClick={() => resendDelivery(detail.log_id)}
                  startIcon={resendingId === detail.log_id ? <CircularProgress size={14} color="inherit" /> : <Icon name="refresh-cw" size={16} />}
                  data-testid="webhook-detail-resend-btn"
                >
                  {resendingId === detail.log_id
                    ? tr("webhook.resending", { defaultValue: "Re-sending…" })
                    : tr("webhook.resendEvent", { defaultValue: "Re-send this event" })}
                </Button>
              </Box>
            </Box>
          )}
