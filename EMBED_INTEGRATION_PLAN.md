...
- [ ] Phase 2 — (c) Buy Button (publishable key + web component + dashboard + tests)
  - [x] **2A backend foundation** (Session 21d): `tbl_publishable_key` model, `POST/GET/PATCH/DELETE /api/publishable-keys` CRUD (JWT), `POST /api/embed/public/session` (pk auth + Origin allow-list + wildcard `*.subdomain` + per-key rate limit 30/min + CORS reflection). CSRF middleware skips `x-publishable-key`. **BACKEND-TESTED 16/16** — incl. all 6 security-critical assertions (bogus Origin → 403, amount > max_amount → 400, wildcard matches subdomains, rate limit → 429, revoked pk → 401, CSRF skip).
  - [x] **2B SDK** (Session 21d): `<dynopay-buy-button>` custom element in `/v1/embed.js` — attrs `publishable-key`, `amount`, `currency?`, `label?`, `mode="modal|redirect|inline"?`, `theme="dark|light"?`, `redirect-uri?`, `meta?`. Emits `dynopay:complete`/`dynopay:error` events. `/embed-test.html` extended with a demo section (activated by `?pk=…`).
