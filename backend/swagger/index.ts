...
import { buildInternalSpec, buildMerchantSpec } from "./specSplit";
...
\`\`\`bash
# Step 1: Login to get JWT token
curl -X POST https://dynopay.com/api/user/login \\
  -H "Content-Type: application/json" \\
  -d '{
...
**API Base URL:** https://dynopay.com/api  
...
---`,
    ...
    components: {
      ...
      schemas: {
        ...
        // First-class payment resource (API review §5.3.1). Returned as
        // `data.payment` by createPayment, cryptoPayment and getPaymentStatus.
        Payment: {
          type: "object",
          description: "First-class payment resource. Returned identically on create and retrieve so a client never has to parse the id out of a URL. Crypto amounts are strings (up to 8dp); fiat amounts are numbers.",
          properties: {
            id: { type: "string", description: "Unique payment id (same value used to poll getPaymentStatus).", example: "a1b2c3d4e5f6..." },
            object: { type: "string", enum: ["payment"], example: "payment" },
            status: {
              type: "string",
              enum: ["waiting", "pending", "confirmed", "processing", "settled", "underpaid", "failed", "expired", "refunded"],
              description: "Payment lifecycle status. Funds have reached the merchant when `settled`.",
              example: "waiting",
            },
            hosted_url: { type: "string", nullable: true, description: "Hosted checkout URL (null for direct crypto payments).", example: "https://checkout.dynopay.com/pay?d=abc123" },
            amount: { type: "number", nullable: true, description: "Fiat amount requested.", example: 100 },
            currency: { type: "string", nullable: true, description: "Fiat currency.", example: "USD" },
            crypto: {
              type: "object",
              nullable: true,
              description: "Crypto details (present once a coin is known). Amounts are strings.",
              properties: {
                amount: { type: "string", nullable: true, example: "0.0031" },
                currency: { type: "string", nullable: true, example: "BTC" },
                address: { type: "string", description: "Deposit address (direct payments)." },
                destination_tag: { type: "integer", description: "XRP/RLUSD destination tag when applicable." },
              },
            },
            fee: { type: "number", nullable: true, description: "Processing fee in fiat.", example: 1.2 },
            tax_amount: { type: "number", nullable: true },
            expires_at: { type: "string", format: "date-time", nullable: true },
            metadata: { type: "object", nullable: true, description: "Merchant-supplied metadata echoed back." },
            created_at: { type: "string", format: "date-time", nullable: true },
            updated_at: { type: "string", format: "date-time", nullable: true },
            redirect_url: { type: "string", nullable: true, description: "Where the buyer is sent after checkout / your success URL." },
            auto_converted: { type: "boolean", description: "Present on retrieve — whether this payment was auto-converted to stablecoin." },
...
const swaggerSpec = swaggerJsdoc(options) as Parameters<typeof buildMerchantSpec>[0];
const merchantSpec = buildMerchantSpec(swaggerSpec);
const internalSpec = buildInternalSpec(swaggerSpec);
const INTERNAL_DOCS_ENABLED = String(envRaw("ENABLE_INTERNAL_API_DOCS") || "").toLowerCase() === "true";
...
const SWAGGER_CSS = `
      ...
      .swagger-ui .dp-copy-wrap { position: relative; margin: 12px 0; }
      .swagger-ui .dp-copy-wrap > pre { margin: 0 !important; }
      .swagger-ui .dp-copy-wrap > pre > code { display: block; box-sizing: border-box; padding-right: 92px !important; }
      .swagger-ui .dp-copy-btn { position: absolute; top: 8px; right: 8px; z-index: 1; display: inline-flex; align-items: center; gap: 4px;
        padding: 5px 12px; border-radius: 8px; border: 1px solid #3b4151; background: #3b4151; color: #fff;
        font: 600 12px/1 -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; cursor: pointer; transition: background-color .15s, border-color .15s; }
      .swagger-ui .dp-copy-btn:hover { background: #1f2532; border-color: #1f2532; }
      .swagger-ui .dp-copy-btn.is-done { background: #16a34a; border-color: #16a34a; }
    `;
// One-tap "Copy" on every fenced code sample in the rendered markdown (quick start,
// auth, webhook guides). Swagger's own example panels already ship a copy control
// (.highlight-code .copy-to-clipboard) and are skipped.
const SWAGGER_COPY_JS = `(function () {
  function copyText(t) {
  ...
  function decorate(pre) {
    if (pre.getAttribute("data-dp-copy") || pre.closest(".highlight-code")) return;
    pre.setAttribute("data-dp-copy", "1");
    var wrap = document.createElement("div"); wrap.className = "dp-copy-wrap";
    pre.parentNode.insertBefore(wrap, pre); wrap.appendChild(pre);
    var btn = document.createElement("button"); btn.type = "button"; btn.className = "dp-copy-btn";
    btn.textContent = "Copy"; btn.setAttribute("aria-label", "Copy code to clipboard"); btn.setAttribute("data-testid", "swagger-copy-btn");
    btn.addEventListener("click", function () {
      var code = pre.querySelector("code"); var text = (code || pre).innerText.replace(/\\s+$/, "");
      copyText(text).then(function () {
        btn.textContent = "Copied"; btn.classList.add("is-done"); btn.setAttribute("data-copied", "true");
        setTimeout(function () { btn.textContent = "Copy"; btn.classList.remove("is-done"); btn.setAttribute("data-copied", "false"); }, 1500);
      }).catch(function () { btn.textContent = "Press Ctrl+C"; setTimeout(function () { btn.textContent = "Copy"; }, 1500); });
    });
    wrap.appendChild(btn);
  }
  function scan() {
    document.querySelectorAll(".swagger-ui .renderedMarkdown pre, .swagger-ui .markdown pre, .swagger-ui .info .description pre").forEach(decorate);
  }
  var timer = null;
  var mo = new MutationObserver(function () { if (timer) return; timer = setTimeout(function () { timer = null; scan(); }, 120); });
  function start() {
    var el = document.getElementById("swagger-ui");
    if (!el) { setTimeout(start, 200); return; }
    scan(); mo.observe(el, { childList: true, subtree: true });
  }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", start); else start();
})();`;
type SwaggerSetupOptions = SwaggerUiOptions & { customJsStr?: string };
const swaggerSetupOptions = (title: string): SwaggerSetupOptions => ({
...
export const setupSwagger = (app: Express) => {
  // Internal/admin reference — registered FIRST so it isn't swallowed by the
  // /api/docs mount below. Off by default in production.
  if (INTERNAL_DOCS_ENABLED) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (app as any).use("/api/docs/internal", swaggerUi.serveFiles(internalSpec, { customCss: SWAGGER_CSS, swaggerOptions: SWAGGER_OPTIONS }),
      swaggerUi.setup(internalSpec, swaggerSetupOptions("Dynopay API — internal reference")));
    app.get("/api/docs/internal.json", (_req, res) => {
      res.setHeader("Content-Type", "application/json");
      res.send(internalSpec);
    });
  } else {
    app.get(["/api/docs/internal", "/api/docs/internal.json"], (_req, res) => {
      res.status(404).json({ message: "Internal API reference is disabled. Set ENABLE_INTERNAL_API_DOCS=true to enable it." });
    });
  }

  // Merchant-facing reference (default)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (app as any).use("/api/docs", swaggerUi.serveFiles(merchantSpec, { customCss: SWAGGER_CSS, swaggerOptions: SWAGGER_OPTIONS }),
    swaggerUi.setup(merchantSpec, swaggerSetupOptions("Dynopay Merchant API Documentation")));

  // Serve raw OpenAPI spec (merchant)
  app.get("/api/docs.json", (_req, res) => {
    res.setHeader("Content-Type", "application/json");
    res.send(merchantSpec);
  });

  log(`Swagger documentation available at /api/docs (merchant, ${Object.keys(merchantSpec.paths).length} paths)${INTERNAL_DOCS_ENABLED ? " + /api/docs/internal" : ""}`, "info");
};

export { merchantSpec, internalSpec };
