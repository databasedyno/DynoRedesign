export const directApiPaths = {
  '/api/user/events': {
    get: {
      tags: ['Direct API - Merchant Integration', 'Webhooks'],
      summary: 'List webhook events (reconciliation)',
      operationId: 'listWebhookEvents',
      description: `List the webhook events Dynopay has delivered to your endpoints so you can reconcile anything your server missed — no need to poll payment status per payment. Cursor-paginated (limit + starting_after + has_more); filter by event type and delivery status.

**🔐 Authentication:** Requires \`x-api-key\` header.`,
      security: [{ ApiKeyAuth: [] }],
      parameters: [
        { in: 'query', name: 'limit', schema: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, description: 'Results per page.' },
        { in: 'query', name: 'starting_after', schema: { type: 'string' }, description: 'Cursor: an event id from a previous page. Returns the next page (newest→oldest). Use the response `next_cursor`.' },
        { in: 'query', name: 'type', schema: { type: 'string' }, description: 'Filter by event name, e.g. payment.confirmed.' },
        { in: 'query', name: 'status', schema: { type: 'string', enum: ['success', 'failed'] }, description: 'Filter by delivery status.' },
      ],
      responses: {
        200: {
            ...
            properties: {
              ...
              data: { type: 'array', items: {
                ...
                properties: {
                  ...
                  event: { type: 'string', example: 'payment.settled' },
  ...
  '/api/user/events/{id}/resend': {
    post: {
      ...
      description: `Re-deliver a single event to its original URL with a fresh webhook_id, timestamp and signature (signed with your current webhook secret). Scoped to your brand.

**🔐 Authentication:** Requires \`x-api-key\` header.`,
      security: [{ ApiKeyAuth: [] }],
      parameters: [
        { in: 'path', name: 'id', required: true, schema: { type: 'integer' }, description: 'The event id from GET /api/user/events.' },
      ],
      responses: {
        200: {
...
**Conversion Statuses:** PENDING_DEPOSIT → DEPOSIT_CREDITED → CONVERTING → CONVERTED → WITHDRAWING → COMPLETED`,
      security: [{ ApiKeyAuth: [], BearerAuth: [] }],
      parameters: [
        { in: 'query', name: 'page', schema: { type: 'integer', default: 1 }, description: 'Page number (legacy offset pagination — still supported)' },
        { in: 'query', name: 'limit', schema: { type: 'integer', default: 10, maximum: 100 }, description: 'Items per page (max 100)' },
        { in: 'query', name: 'starting_after', schema: { type: 'string' }, description: 'Cursor pagination: a payment_id from a previous page. Returns the next page of results after that item (ordered newest→oldest). Use the response `next_cursor` to fetch the following page.' },
      ],
      responses: {
        200: {
            ...
            'application/json': {
              schema: {
                ...
                properties: {
                  ...
                  has_more: { type: 'boolean', description: 'True when more results exist after this page (cursor + offset modes).' },
                  next_cursor: { type: 'string', nullable: true, description: 'Pass as `starting_after` to fetch the next page. Null on the last page.' },
                  limit: { type: 'integer', description: 'Effective page size applied.' },
                  data: {
                    type: 'array',
                    items: {
                      type: 'object',
...
**💡 USE CASE:** Check wallet configuration before offering payment options to customers`,
  ...
  '/api/user/createPayment': {
    post: {
      ...
      responses: {
        200: { description: 'Payment link created', content: { 'application/json': { schema: { type: 'object', properties: { success: { type: 'boolean' }, message: { type: 'string' }, data: { type: 'object', properties: { redirect_url: { type: 'string', description: 'Hosted checkout URL.' }, fee_payer: { type: 'string' }, available_currencies: { type: 'array', items: { type: 'string' } }, webhook_url: { type: 'string' }, payment: { $ref: '#/components/schemas/Payment' } } } } } } } },
        ...
        200: {
          ...
          content: { 'application/json': { schema: { type: 'object', properties: {
            ...
            data: { type: 'object', properties: {
              ...
              amount: { type: 'number', description: 'Full crypto amount expected for this payment' },
              currency: { type: 'string' },
              amount_received: { type: 'number', description: 'Crypto amount actually received so far. Equals the full amount once paid; the partial amount while underpaid.' },
              amount_remaining: { type: 'number', description: 'Crypto amount still owed (0 once fully paid). Non-zero only while underpaid.' },
              paid_amount: { type: 'number', description: 'Alias of amount_received (crypto received so far).' },
              amount_received_base: { type: 'number', description: 'amount_received expressed in base_currency (e.g. USD, EUR).' },
              amount_remaining_base: { type: 'number', description: 'amount_remaining expressed in base_currency (e.g. USD, EUR).' },
              base_amount: { type: 'number' },
              base_currency: { type: 'string' },
