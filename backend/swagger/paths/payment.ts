...
**Response includes company_id** for client-side filtering if needed.`,
      ...
      parameters: [
        ...
        { in: 'query', name: 'page', schema: { type: 'integer', default: 1 }, description: 'Legacy offset pagination (still supported).' },
        { in: 'query', name: 'limit', schema: { type: 'integer', default: 20, maximum: 100 }, description: 'Items per page (max 100).' },
        { in: 'query', name: 'starting_after', schema: { type: 'string' }, description: 'Cursor pagination: a link_id from a previous page. Returns the next page after that item (newest→oldest). Use the response `pagination.next_cursor` to fetch the following page.' }
      ],
      responses: {
        200: {
            ...
            'application/json': {
              example: {
                ...
                data: {
                  ...
                  pagination: { total: 25, page: 1, limit: 20, totalPages: 2, has_more: true, next_cursor: 421 }
                }
              }
            }
          }
