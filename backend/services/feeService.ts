...
import { getCoinMinimumUsd } from "./checkout/checkoutMinimums";
...
export const getBlockchainConfig = async (blockchain: string, userId?: number) => {
  ...
  if (threshold !== undefined && tiers.length > 0) {
    // Align the settlement/sweep "min forwarding" gate to the SAME live-fee
    // source the checkout coin-picker uses (checkoutMinimums.getCoinMinimumUsd =
    // ceil(liveGasUSD × multiple), static sweep floor as fallback). Founder-
    // signed (2026-09-14). We take max(staticThreshold, liveCheckoutMin) so a
    // gas spike can RAISE the bar (never forward a payment that can't cover its
    // own sweep) but the historical flat floor is never lowered. Gated by
    // ALIGN_SETTLEMENT_MIN_TO_CHECKOUT (default on) as an ops kill-switch.
    let minForwarding = Number(threshold);
    if (String(process.env.ALIGN_SETTLEMENT_MIN_TO_CHECKOUT ?? "true").toLowerCase() !== "false") {
      try {
        const liveMin = await getCoinMinimumUsd(blockchain);
        if (Number.isFinite(liveMin) && liveMin > 0) {
          minForwarding = Math.max(minForwarding, Number(liveMin));
        }
      } catch (e) {
        log(`[getBlockchainConfig] live-fee min lookup failed for ${blockchain}, using static threshold: ${(e as Error).message}`, 'warn');
      }
    }

    return {
      blockchain,
      min_forwarding_amount: minForwarding,
