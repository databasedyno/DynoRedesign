...
export const releaseAddress = async (
  ...
  // Fresh funds landed on this address → any "unprofitable dust" sweep deferral is stale.
  // Without this, an auto-convert payment on a previously-deferred address is never swept
  // (root cause of the Sep-2026 stuck ETH conversions).
  if (newAdminBalance > currentAdminBalance && adminFeeAmount > 0) {
    try {
      await deleteRedisItem(`sweep:unprofitable:${tempAddressId}`);
