...
export const sendWelcomeEmail = async (
  ...
  try {
    ...
    const html = dynoPayEmailTemplate(t('merchant.welcome.heading', L), content, true, t('merchant.welcome.cta', L), `${FRONTEND_BASE_URL}/dashboard`, t('merchant.welcome.preheader', L), L, 'rocket');
...
/**
 * Purpose-aware OTP email (sendPurposeOTPEmail) lives in ./otpEmails.ts —
 * extracted so this file stays lean.
 */
...
export const sendSecurityAlertEmail = async (
  ...
  try {
    ...
    const html = dynoPayEmailTemplate(t('merchant.securityAlert.heading', L), content, true, t('merchant.securityAlert.cta', L), `${FRONTEND_BASE_URL}/settings`, t('merchant.securityAlert.preheader', L), L, 'shield-alert');
    await mailTransporter({ to: email, name, subject, body: html });
...
export const sendFailedLoginAttemptsEmail = async (
  ...
  try {
    ...
    const html = dynoPayEmailTemplate(t('merchant.failedLogins.heading', L), content, true, t('merchant.failedLogins.cta', L), `${FRONTEND_BASE_URL}/auth/login`, t('merchant.failedLogins.preheader', L), L, 'danger');
    await mailTransporter({ to: email, name, subject, body: html });
...
export const sendNewDeviceAlertEmail = async (
  ...
  try {
    const { ipAddress, device, browser, os, location, at, securityToken } = opts;
    const L = await resolveEmailLang(opts.lang, email);
    const K = 'merchant.newDeviceAlert';
    ...
    const deviceDisplay =
      [device, browser, os].filter((x) => x && x !== "Unknown").join(" · ") || t(`${K}.unknownDevice`, L);
    const locationDisplay = location || t(`${K}.unknownLocation`, L);
    ...
    ${infoBox(`
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        ${dataRow(t('merchant.labels.device', L), escapeHtml(deviceDisplay))}
        ${dataRow(t('merchant.labels.location', L), escapeHtml(locationDisplay))}
        ${dataRow(t('merchant.labels.ipAddress', L), `<span style="font-family: monospace; font-size: 13px;">${escapeHtml(ipAddress)}</span>`)}
        ${dataRow(t(`${K}.when`, L), escapeHtml(formatEmailDateTime(at, L)), true)}
      </table>
    `)}
    ${warnText(t(`${K}.warn`, L))}`;
    ...
    await mailTransporter({ to: email, name, subject, body: html });
    apiLogger.info(`[Email] New-device alert sent to ${email} (${deviceDisplay}, ${locationDisplay}, lang=${L})`);
  } catch (e) {
