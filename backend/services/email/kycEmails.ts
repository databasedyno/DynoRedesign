...
export const sendKYCRequiredEmail = async (
  ...
  try {
    ...
    const html = dynoPayEmailTemplate(t('merchant.kycRequired.heading', L), content, true, t('merchant.kycRequired.cta', L), `${FRONTEND_BASE_URL}/kyc`, t('merchant.kycRequired.preheader', L, { threshold: thresholdAmount }), L, 'id-card');
...
export const sendKYCRejectedEmail = async (email: string, name: string, rejectionReason: string, lang?: string) => {
  try {
    ...
    const html = dynoPayEmailTemplate(t('merchant.kycRejected.heading', L), content, true, t('merchant.kycRejected.cta', L), `${FRONTEND_BASE_URL}/kyc`, t('merchant.kycRejected.preheader', L), L, 'id-card-red');
    await mailTransporter({ to: email, name, subject, body: html });
...
export const sendKYCResubmissionRequiredEmail = async (email: string, name: string, reason: string, lang?: string) => {
  try {
    ...
    const html = dynoPayEmailTemplate(t('merchant.kycResubmission.heading', L), content, true, t('merchant.kycResubmission.cta', L), `${FRONTEND_BASE_URL}/kyc`, t('merchant.kycResubmission.preheader', L), L, 'id-card-red');
    await mailTransporter({ to: email, name, subject, body: html });
