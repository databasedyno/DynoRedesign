...
function renderOrderItemsTable(
    ...
    .map((it: any) => {
      ...
      if (opts.includeDeliveryLinks && it.delivered_payload) {
        ...
        if (Array.isArray(dp.asset_deliveries) && dp.asset_deliveries.length > 0) {
          ...
          deliveryHtml = `<div class="chip" style="margin-top:6px;font-family:monospace;background:#f3f4f6;padding:6px 8px;border-radius:6px;font-size:13px;">${tr('orderTable.licenseKey', L)} ${esc(dp.license_key)}</div>`;
  ...
  return `
    <table class="tbl-surface" style="width:100%;border-collapse:collapse;margin:16px 0;background:#ffffff;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
      <thead>
        <tr class="tbl-head" style="background:#f9fafb;">
...
export const sendOrderReceiptMerchantEmail = async (
  ...
  try {
    ...
    const shortRef = String(order.public_ref || "").slice(0, 8).toUpperCase();
    const subject = brandSubject(companyName, `New sale – ${formatCents(order.total_cents, order.currency || "USD")}`);
...
export const sendOrderExpiredEmail = async (
  ...
  try {
    ...
    const shortRef = String(order.public_ref || "").slice(0, 8).toUpperCase();
    const subject = `Your order ${shortRef} wasn't completed`;
...
export const sendOrderRefundedEmail = async (
  ...
  try {
    const name = buyerName || buyerEmail || "there";
    const shortRef = String(order.public_ref || "").slice(0, 8).toUpperCase();
    const subject = `Refund confirmed for order ${shortRef}`;
    const itemsTable = renderOrderItemsTable(order, items, { includeDeliveryLinks: false });
...
export const sendOrderShippedEmail = async (
  ...
  try {
    const name = buyerName || buyerEmail || "there";
    const shortRef = String(order.public_ref || "").slice(0, 8).toUpperCase();
    const subject = `Your order ${shortRef} has shipped`;
...
export const sendDigitalDownloadReminderEmail = async (
  ...
  try {
    const name = buyerName || buyerEmail || "there";
    const shortRef = String(order.public_ref || "").slice(0, 8).toUpperCase();
    const subject = `Your download links expire soon (order ${shortRef})`;
    const itemsTable = renderOrderItemsTable(order, items, { includeDeliveryLinks: false });
