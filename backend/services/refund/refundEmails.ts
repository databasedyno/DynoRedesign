...
export const sendRefundStatusEmail = async (refund: any): Promise<void> => {
  try {
    ...
    // Resolve the merchant's brand so the CUSTOMER knows who refunded them
    // (a Dynopay account can run several brands). Best-effort — never block send.
    let brandName = "";
    try {
      if (r.company_id) {
        const companyRow: any = await companyModel.findByPk(r.company_id);
        brandName = (await resolvePublicCompanyName(companyRow?.dataValues || companyRow)) || "";
      }
    } catch (e: any) {
      log(`[refundEmails] brand name resolve failed: ${e?.message || e}`, "error");
    }

    // 1) Customer receipt
    if (customerEmail) {
      try {
        const { subject, html } = buildRefundEmail({ ...r, brand_name: brandName }, kind);
