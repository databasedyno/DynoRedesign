  ...
  try {
    ...
    try {
      ...
      if (merchant?.dataValues?.email) {
        // Brand name for the subject line so a multi-brand merchant can tell
        // which brand the sale belongs to straight from their inbox.
        let brandName: string | null = null;
        try {
          const { default: companyModel } = await import("../models/companyModels/companyModel");
          const cid = (order.dataValues as any).company_id ?? null;
          if (cid) {
            const co: any = await companyModel.findByPk(cid);
            brandName = co?.dataValues?.company_name || null;
          }
        } catch (coErr: any) {
          apiLogger.warn(
            `[fulfillment] brand lookup for merchant sale email failed: ${coErr?.message || coErr}`,
          );
        }
