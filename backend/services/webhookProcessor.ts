...
// Persist the crypto received/remaining (and optionally an underpaid status) on
// the merchant transaction so getPaymentStatus can report partial amounts even
// after the short-lived Redis checkout session expires. Fire-and-forget: any
// failure is logged but never blocks the settlement/webhook path.
function persistUnderpaymentAmounts(
  ...
  (async () => {
    try {
      const { userTransactionModel } = await import("../models");
      const update: Record<string, unknown> = {
        received_amount: Number(receivedAmount),
        remaining_amount: Math.max(0, Number(remainingAmount)),
      };
      if (status) update.status = status;
      await userTransactionModel.update(update, { where: { id: String(paymentId) } });
    } catch (e) {
      webhookLogs.error(`[WebhookProcessor] persistUnderpaymentAmounts failed for ${paymentId}:`, e);
    }
  })();
}
...
async function handleNewTransaction(
  ...
  if (isUnderpayment && !isMinorUnderpayment) {
    ...
    if (isDirectApi) {
      ...
      // Persist received/remaining crypto to the DB so getPaymentStatus can
      // expose amount_received / amount_remaining after the Redis session
      // expires (fire-and-forget — never block settlement).
      persistUnderpaymentAmounts(items?.payment_id || items?.unique_tx_id, totalReceivedAmount, expectedAmount - totalReceivedAmount);
      ...
      // Persist received/remaining crypto + underpaid status to the DB so a
      // merchant polling getPaymentStatus sees the partial amounts even after
      // the short-lived Redis session expires (fire-and-forget).
      persistUnderpaymentAmounts(items?.payment_id || items?.unique_tx_id, totalReceivedAmount, remainingAmount, "underpaid");
      ...
      // Nudge the BUYER: "you're almost there — send X more to finish" while the
      // deposit address is still open. Fire-and-forget + deduped per address.
      sendBuyerUnderpaidNudge(
        address,
        totalReceivedAmount,
        expectedAmount,
        items?.currency || payload.asset,
        { ...(customerData || {}), payment_ref: items?.payment_id || items?.unique_tx_id },
        merchantGracePeriodMinutes
      ).catch((e) => webhookLogs.error("[WebhookProcessor] sendBuyerUnderpaidNudge failed:", e));
