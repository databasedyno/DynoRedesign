...
// ── Read everything: all comments (grouped by item) + custom items ─────────
router.get("/data", requirePasscode, (_req, res) =>
  withTables(res, async () => {
    const [comments, customItems] = await Promise.all([
    ...
    res.json({ ok: true, commentsByItem: byItem, customItems });
  })
);
