 ...
 * Approved KYC on the company itself OR an account-level approval both count.
 */
export async function isMerchantIdentityVerified(
  ...
  try {
    const where =
      companyId != null
        ? "user_id = :userId AND status = 'approved' AND (company_id = :companyId OR company_id IS NULL)"
        : "user_id = :userId AND status = 'approved' AND company_id IS NULL";
    const rows = (await sequelize.query(
      `SELECT 1 FROM tbl_kyc WHERE ${where} LIMIT 1`,
      {
        replacements: { userId, companyId: companyId ?? null },
