...
/**
 * Identity verification is ACCOUNT-level: Veriff verifies the person, not the
 * brand. An APPROVED record for the user — under any brand or none — is
 * authoritative for every brand they own. Otherwise fall back to the most
 * recent record scoped to the requested brand (in-flight / declined state).
 */
export async function findEffectiveKycRecord(
  userId: string | number,
  companyId?: string | number | null,
) {
  const approved = await kycModel.findOne({
  ...
  return kycModel.findOne({ where, order: [["created_at", "DESC"]] });
}
...
export async function checkKycEnforcement(
  ...
  const volumeQuery = companyId
    ...
    : `SELECT COALESCE(SUM(CAST(base_amount AS DECIMAL)), 0) as total_volume 
       FROM tbl_customer_transaction 
       WHERE company_id IN (SELECT company_id FROM tbl_company WHERE user_id = :userId AND deleted_at IS NULL) AND status = 'successful'`;
  ...
  // KYC is required — check status (account-level: approved anywhere counts)
