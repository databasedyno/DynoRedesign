...
def ensure_node_modules():
    ...
    # --prefer-offline: use the warm yarn cache, skip the flaky registry fetch that
    # left the first pass incomplete and forced the slow --check-files re-run.
    for extra in (['--production=false', '--prefer-offline', '--network-concurrency', '16'], ['--production=false', '--prefer-offline', '--check-files']):
        subprocess.run(
