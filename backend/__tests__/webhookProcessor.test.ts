...
describe('Webhook Processor — processWebhookJob', () => {
  ...
  describe('Stage 9 — Merchant-configured tolerance (Settings › Payments)', () => {
    const seedLink = () => {
      seedRedis('crypto-0xTestAddress', createRedisPaymentData({
        txId: undefined,
        amount: '100',
        base_amount: '100', // $100 link → 1 crypto unit == $1
        link_id: 'link-001',
      }));
      seedRedis('ref-001', { adm_id: 1, company_id: 1, link_id: 'link-001' });
    };

    it('accepts a $3 shortfall when the merchant allows underpayments up to $5', async () => {
      seedLink();
      (companyModel.findOne as jest.Mock).mockResolvedValue({
        dataValues: { underpayment_threshold_usd: '5.00', grace_period_minutes: 30 },
      });

      await processWebhookJob(createJobData({ amount: '97' }));

      expect(paymentController.cryptoVerification).toHaveBeenCalled();
      expect(setRedisItem).not.toHaveBeenCalledWith('crypto-0xTestAddress', expect.objectContaining({ status: 'underpaid' }));
    });

    it('still marks underpaid when the shortfall ($6) exceeds the merchant threshold ($5)', async () => {
      seedLink();
      (companyModel.findOne as jest.Mock).mockResolvedValue({
        dataValues: { underpayment_threshold_usd: '5.00', grace_period_minutes: 30 },
      });

      await processWebhookJob(createJobData({ amount: '94' }));

      expect(paymentController.cryptoVerification).not.toHaveBeenCalled();
      expect(setRedisItem).toHaveBeenCalledWith(
        'crypto-0xTestAddress',
        expect.objectContaining({ status: 'underpaid', incomplete: 'true', receivedAmount: 94, amount: 6 })
      );
    });

    it('rejects a $3 shortfall when the merchant threshold is $0 (strict)', async () => {
      seedLink();
      (companyModel.findOne as jest.Mock).mockResolvedValue({
        dataValues: { underpayment_threshold_usd: '0', grace_period_minutes: 30 },
      });
      ...
      expect(setRedisItem).toHaveBeenCalledWith(
        'crypto-0xTestAddress',
        expect.objectContaining({ status: 'underpaid' })
      );
    });

    it('uses the merchant grace period for the partial-payment TTL (10 min → 600 s) and tells the merchant', async () => {
      seedLink();
      (companyModel.findOne as jest.Mock).mockResolvedValue({
        dataValues: { underpayment_threshold_usd: '1.00', grace_period_minutes: 10 },
      });

      await processWebhookJob(createJobData({ amount: '70' }));

      expect(setRedisTTL).toHaveBeenCalledWith('crypto-0xTestAddress', 600);
      expect(callMerchantWebhook).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ event: 'payment.underpaid', grace_period_minutes: 10 })
      );
    });

    it('caps an out-of-range stored grace period at 30 minutes', async () => {
      seedLink();
      (companyModel.findOne as jest.Mock).mockResolvedValue({
        dataValues: { underpayment_threshold_usd: '1.00', grace_period_minutes: 90 },
      });
    ...
    it('never applies the tolerance to Direct API payments (settles the received amount instead)', async () => {
      seedRedis('crypto-0xTestAddress', createRedisPaymentData({ txId: undefined, amount: '100', base_amount: '100' }));
      seedRedis('ref-001', { adm_id: 1, company_id: 1 });
      (companyModel.findOne as jest.Mock).mockResolvedValue({
        dataValues: { underpayment_threshold_usd: '5.00', grace_period_minutes: 30 },
      });
      ...
      expect(callMerchantWebhook).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ event: 'payment.underpaid', payment_type: 'direct_api' })
      );
    });
  });
