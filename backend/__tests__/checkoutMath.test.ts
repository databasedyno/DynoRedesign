import { computeCheckoutSplit, computeFallbackSplit, computeInclusiveSplit, computeReceivedSplit } from "../controller/payment/checkoutMath";
...
describe("checkoutMath.computeReceivedSplit (settlement of the amount actually received)", () => {
  const ties = (s: { merchantAmount: number; feeAmount: number }, received: string | number) =>
    D(s.merchantAmount).plus(s.feeAmount).eq(D(received));

  it("tx 942 — buyer overpaid 0.0081762 ETH by sending 0.00993: excess goes to the merchant, fee stays the quoted 0.00049058", () => {
    const s = computeReceivedSplit({ received: "0.00993", expected: "0.0081762", plannedMerchant: "0.00768562" });
    expect(s.overpaymentExcess).toBe(0.0017538);
    expect(s.feeAmount).toBe(0.00049058);
    expect(s.merchantAmount).toBe(0.00943942);
    expect(ties(s, "0.00993")).toBe(true);
    expect(s.paymentRatio).toBeCloseTo(1.2145, 4);
  });

  it("exact payment — planned split unchanged", () => {
    const s = computeReceivedSplit({ received: "0.0213382", expected: "0.0213382", plannedMerchant: "0.0206863" });
    expect(s.overpaymentExcess).toBe(0);
    expect(s.merchantAmount).toBe(0.0206863);
    expect(s.feeAmount).toBe(0.0006519);
  });

  it("tx 944 — slight underpayment inside tolerance scales the merchant share down, no excess", () => {
    const s = computeReceivedSplit({ received: "0.0213092", expected: "0.0213382", plannedMerchant: "0.0206863" });
    expect(s.overpaymentExcess).toBe(0);
    expect(s.merchantAmount).toBe(0.02065818);
    expect(s.feeAmount).toBe(0.00065102);
    expect(ties(s, "0.0213092")).toBe(true);
  });

  it("fee is never negative even when the planned merchant share exceeds the expected total", () => {
    const s = computeReceivedSplit({ received: "0.02", expected: "0.01", plannedMerchant: "0.011" });
    expect(s.feeAmount).toBe(0);
    expect(s.merchantAmount).toBe(0.02);
  });

  it("randomised: always ties, fee ≥ 0, excess == received − expected when overpaid", () => {
    let seed = 7;
    const rnd = () => (seed = (seed * 1103515245 + 12345) % 2147483648) / 2147483648;
    for (let k = 0; k < 500; k++) {
      const expected = (rnd() + 0.0001).toFixed(8);
      const planned = (Number(expected) * (0.9 + rnd() * 0.09)).toFixed(8);
      const received = (Number(expected) * (0.5 + rnd())).toFixed(8);
      const s = computeReceivedSplit({ received, expected, plannedMerchant: planned });
      expect(ties(s, received)).toBe(true);
      expect(s.feeAmount).toBeGreaterThanOrEqual(0);
      if (Number(received) > Number(expected)) expect(D(s.overpaymentExcess).eq(D(received).minus(expected))).toBe(true);
      else expect(s.overpaymentExcess).toBe(0);
    }
  });
});
