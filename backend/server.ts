...
import { sendMerchantUnderpaidDigest } from "./services/pendingPaymentService";
...
// Public assets (email logo/hero/social PNGs, product images, videos) are embedded
// cross-origin by mail clients (Apple Mail / Outlook load <img> directly) and by
// storefront embeds. helmet's default Cross-Origin-Resource-Policy: same-origin
// makes browsers/WebViews BLOCK those loads (ERR_BLOCKED_BY_RESPONSE), so relax it
// for the static tree only. Long-lived cache for immutable brand assets.
app.use("/api/static", (_req, res, next) => {
  res.setHeader("Cross-Origin-Resource-Policy", "cross-origin");
  next();
});
app.use("/api/static", express.static("public", { maxAge: "7d" }));
...
// Brand soft-delete purge — permanently delete brands whose 7-day recovery
// window has elapsed (services/brandPurgeService). Daily at 03:20 UTC.
leaderCron.schedule("20 3 * * *", async () => {
  const lockAcquired = await acquireLock("cron:purgeExpiredBrands", 600, 1, 100, true);
  if (!lockAcquired) return;
  try {
    ...
    captureError(err as Error, 'cron', { extraContext: 'purgeExpiredBrands' });
  } finally {
    await releaseLock("cron:purgeExpiredBrands");
  }
});

// Account soft-delete purge — permanently delete accounts whose 7-day recovery
// window has elapsed (services/accountPurgeService). Daily at 03:35 UTC.
leaderCron.schedule("35 3 * * *", async () => {
  const lockAcquired = await acquireLock("cron:purgeExpiredAccounts", 600, 1, 100, true);
  if (!lockAcquired) return;
  try {
    ...
    captureError(err as Error, 'cron', { extraContext: 'purgeExpiredAccounts' });
  } finally {
    await releaseLock("cron:purgeExpiredAccounts");
  }
});

// Brand delete reminder — day-5 nudge for brands ~2 days from permanent purge
// (services/brandPurgeService). Daily at 09:15 UTC (a friendlier send time).
leaderCron.schedule("15 9 * * *", async () => {
  const lockAcquired = await acquireLock("cron:remindExpiringBrands", 600, 1, 100, true);
  if (!lockAcquired) return;
  try {
    const result = await remindExpiringBrands();
    if (result.scanned > 0) {
      log(`Cron: remindExpiringBrands — scanned ${result.scanned}, reminded ${result.reminded}`, "info");
    }
  } catch (err) {
    log(`Cron: remindExpiringBrands failed: ${(err as Error).message}`, "error");
    captureError(err as Error, 'cron', { extraContext: 'remindExpiringBrands' });
  } finally {
    await releaseLock("cron:remindExpiringBrands");
  }
});

// Merchant underpaid digest — once daily at 08:00 UTC, email each brand a
// summary of payments that came up short in the last 24h so shortfalls don't
// slip through. Sends nothing to brands with zero underpayments.
// See services/pendingPaymentService.ts → sendMerchantUnderpaidDigest.
leaderCron.schedule("0 8 * * *", async () => {
  const lockAcquired = await acquireLock("cron:underpaidDigest", 600, 1, 100, true);
  if (!lockAcquired) return;
  try {
    const result = await sendMerchantUnderpaidDigest();
    if (result.emails > 0) {
      log(`Cron: underpaidDigest — ${result.companies} companies, ${result.emails} emails sent`, "info");
    }
  } catch (err) {
    log(`Cron: underpaidDigest failed: ${(err as Error).message}`, "error");
    captureError(err as Error, 'cron', { extraContext: 'underpaidDigest' });
  } finally {
    await releaseLock("cron:underpaidDigest");
  }
});
...
const startServer = async () => {
  ...
  try {
    ...
    // Phase 1 security rollout (prod primary only): wipe legacy API-key plaintext
    // + one-time merchant notice. No-op on preview / secondary. Fire-and-forget.
    setImmediate(async () => {
      const { runApiKeyHashingRollout } = await import("./services/apiKeys/apiKeyHashingRollout");
      await runApiKeyHashingRollout({ isProduction, enableBackgroundJobs, workerRole });
    });
