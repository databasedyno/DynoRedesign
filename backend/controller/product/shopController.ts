  ...
  try {
    ...
    if (STOREFRONT_PER_COMPANY) {
      shops = (await sequelize.query(
        `SELECT LOWER(c.handle) AS handle
           FROM tbl_company c
          WHERE c.handle IS NOT NULL AND c.handle <> ''
            AND COALESCE(c.store_enabled, true) = true
            AND EXISTS (SELECT 1 FROM tbl_product p
                         WHERE p.company_id = c.company_id AND p.status = 'live' AND p.deleted_at IS NULL)
          GROUP BY LOWER(c.handle)
      ...
      shops = (await sequelize.query(
        `SELECT LOWER(u.handle) AS handle
           FROM tbl_user u
          WHERE u.handle IS NOT NULL AND u.handle <> ''
            AND COALESCE(u.store_enabled, true) = true
            AND EXISTS (SELECT 1 FROM tbl_product p
                         WHERE p.merchant_user_id = u.user_id AND p.status = 'live' AND p.deleted_at IS NULL)
