...
import { getEffectiveMinOrderUsd, getMerchantMinOrderUsdByCompanyId } from "../../services/checkout/orderMinimums";
...
export const startCheckout = async (
  ...
  try {
    ...
    // ── Phase 1b: per-brand MINIMUM ORDER enforced store-side ──────────────
    // Reject a cart whose subtotal is below the merchant's effective 'store'
    // minimum BEFORE any stock is decremented, so the buyer gets a clear message
    // here instead of only the coin-picker greying on /pay. USD basis (min is
    // USD); convert the cart's subtotal when priced in another currency.
    try {
      const merchantMinUsd = await getMerchantMinOrderUsdByCompanyId(merchantCompanyId);
      const effectiveMinUsd = getEffectiveMinOrderUsd("store", merchantMinUsd);
      if (effectiveMinUsd > 0) {
        const subtotalMajor = Number(validated.subtotal_cents) / 100;
        let subtotalUsd = subtotalMajor;
        const cur = String(validated.currency || "USD").toUpperCase();
        if (cur !== "USD") {
          try {
            const conv = await convertToFiat(cur, "USD", subtotalMajor);
            if (conv?.amount && Number(conv.amount) > 0) subtotalUsd = Number(conv.amount);
          } catch { /* fall back to raw subtotal on conversion failure */ }
        }
        // 1-cent tolerance for rounding on the boundary.
        if (subtotalUsd + 0.01 < effectiveMinUsd) {
          return errorResponseHelper(
            res,
            400,
            `This store's minimum order is $${effectiveMinUsd.toFixed(2)}. Please add more items to continue.`
          );
        }
      }
    } catch (minErr) {
      apiLogger.warn(`[cartCheckout] min-order check skipped: ${(minErr as Error).message}`);
    }
