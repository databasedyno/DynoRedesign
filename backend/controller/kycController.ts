...
import { isKycExempt, checkKycEnforcement, findEffectiveKycRecord, KYC_THRESHOLD_USD, KYC_GRACE_PERIOD_DAYS } from "../helper/kycEnforcement";
...
const getKYCStatus = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Account-level: an approved record under ANY brand verifies every brand.
...
const startKYCVerification = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Identity is verified per ACCOUNT — an approved record under any brand
    // means there is nothing left to verify for this brand either.
    const existingKYC = await kycModel.findOne({
      where: { user_id: userId, status: "approved" },
    });

    if (existingKYC) {
      return errorResponseHelper(res, 400, "Identity already verified for this account");
    }
...
const resubmitKYC = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Find existing KYC record (account-level: an approved record anywhere
    // surfaces as "approved" and blocks a pointless resubmission)
    const existingKYC = await findEffectiveKycRecord(userId, company_id);
