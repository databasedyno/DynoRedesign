...
import {
  API_KEY_VERSION_TOKEN,
  apiKeyHint,
  generateApiKeyToken,
  hashApiKey,
} from "../helper/apiKeyToken";
...
const addApi = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Opaque random token — the merchant sees it exactly once (this response);
    // we persist only its sha256 + a display hint.
    const plaintextKey = generateApiKeyToken(environment);
    const keyHash = hashApiKey(plaintextKey);
    const keyHint = apiKeyHint(plaintextKey, environment);
    ...
    successResponseHelper(res, 200, successMessage, {
      ...
      // Shown ONCE — never retrievable again (only key_hash is stored).
      apiKey: plaintextKey,
      key_hint: keyHint,
...
const updateApi = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Fetch updated record (never echo the credential column or its hash)
...
const regenerateApiKey = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // New opaque token — preserve environment prefix (dpk_test_ / dpk_live_) so
    // the regenerated key retains the same sandbox vs live semantics. Only the
    // hash is stored; the previous key (legacy ciphertext or token) dies here.
    ...
    const newApiKey = generateApiKeyToken(environment);
    const keyHint = apiKeyHint(newApiKey, environment);
    ...
    successResponseHelper(res, 200, "API key regenerated successfully", {
      ...
      key_hint: keyHint,
      environment,
      message: "Copy this key now — it will not be shown again. Update your integrations with the new API key."
    });
  } catch (e) {
