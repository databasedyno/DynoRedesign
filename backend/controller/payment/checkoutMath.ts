...
export interface ReceivedSplitInput {
  /** Total crypto actually received on the address (cumulative for partial payments). */
  received: MoneyInput;
  /** Crypto the checkout asked for (the ORIGINAL expected total, not the remaining part). */
  expected: MoneyInput;
  /** Merchant share planned at payment creation for the expected total. */
  plannedMerchant: MoneyInput;
  dp?: number;
}
...
export interface ReceivedSplit {
  merchantAmount: number;
  feeAmount: number;
  /** Crypto sent above `expected` — credited to the merchant in full (0 when not overpaid). */
  overpaymentExcess: number;
  paymentRatio: number;
}

/**
 * Settlement split for what was ACTUALLY received.
 *   underpaid / exact (ratio ≤ 1): merchant share scales with the ratio (rounded down), fee is the remainder
 *   overpaid   (ratio > 1):        the quoted fee is charged once, on the expected amount; the
 *                                  ENTIRE excess goes to the merchant (Coinbase Commerce / BTCPay behaviour)
 * Invariant at `dp`: merchantAmount + feeAmount === received; feeAmount is never negative.
 */
export const computeReceivedSplit = (i: ReceivedSplitInput): ReceivedSplit => {
  const dp = i.dp ?? 8;
  const receivedD = roundTo(i.received, dp);
  const expectedD = D(i.expected);
  const plannedD = D(i.plannedMerchant);
  const ratio = expectedD.gt(0) ? div(receivedD, expectedD) : D(1);
  const excessD = ratio.gt(1) ? receivedD.minus(expectedD) : D(0);
  const scaled = roundTo(plannedD.times(ratio.gt(1) ? D(1) : ratio), dp, "down");
  let merchantD = scaled.plus(excessD);
  if (merchantD.gt(receivedD)) merchantD = receivedD;
  if (merchantD.lt(0)) merchantD = D(0);
  return {
    merchantAmount: merchantD.toNumber(),
    feeAmount: receivedD.minus(merchantD).toNumber(),
    overpaymentExcess: excessD.toNumber(),
    paymentRatio: ratio.toNumber(),
  };
};
