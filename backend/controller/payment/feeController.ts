...
import { getCoinMinimumsUsd, getOrderMinimumUsd } from "../../services/checkout/checkoutMinimums";
...
export const getConfiguredCurrenciesForCheckout = async (
  ...
  try {
    ...
    // ── Smart minimums ──────────────────────────────────────────────────
    // Per-coin forwarding minimum (USD) + this order's value in USD, so the
    // checkout can grey out coins the payer can't complete and show each coin's
    // minimum up-front (mirrors the settlement forwarding gate). Additive.
    response.coin_minimums = await getCoinMinimumsUsd(currencies);
    response.min_order_usd = await getOrderMinimumUsd(currencies);
    // Raise per-coin minimums by the merchant's per-brand floor (Phase 1b) so the
    // checkout greys coins/amounts below the merchant's minimum too.
    const merchantMinUsd = await getMerchantMinOrderUsdByCompanyId(companyId);
    if (merchantMinUsd > 0) {
      const cm = response.coin_minimums as Record<string, number>;
      for (const k of Object.keys(cm)) cm[k] = Math.max(Number(cm[k]) || 0, merchantMinUsd);
      response.min_order_usd = Math.max(Number(response.min_order_usd) || 0, merchantMinUsd);
    }
    let transactionAmountUsd = transactionAmount;
    if (transactionCurrency && transactionCurrency.toUpperCase() !== 'USD' && transactionAmount > 0) {
      try {
        const usdConv = await currencyConvert({
    ...
    response.transaction_amount_usd = toNumber(transactionAmountUsd, 2);
