...
export const createPaymentLink = async (
  ...
  try {
    ...
    try {
      // Resolve the brand this link/campaign belongs to so the merchant
      // notification names it — owners frequently manage several brands and the
      // email previously omitted which one entirely.
      let brandName = "";
      if (company_id) {
        try {
          const brandRow = await companyModel.findByPk(company_id);
          brandName = (brandRow as { company_name?: string } | null)?.company_name || "";
        } catch { /* best-effort — never block link creation on this */ }
      }
...
export const getPaymentLinks = async (req: express.Request, res: express.Response) => {
  const userData = jwt.decode(res.locals.token) as PaymentUserJwtPayload;
  try {
    const { company_id, page, limit, paginated, starting_after } = req.query;  // Added pagination params
    ...
    const usePagination = paginated === 'true' || page !== undefined || limit !== undefined || starting_after !== undefined;
    ...
    // Cursor pagination (API review §5.3.3): `starting_after=<link_id>` walks the
    // list by keyset over (createdAt, link_id) DESC — stable under new inserts.
    // Legacy `page`/`limit` offset mode still works unchanged. Fetch one extra
    // row to compute `has_more`.
    const useCursor = typeof starting_after === 'string' && starting_after.length > 0;
    let listWhere: Record<string, unknown> = whereClause;
    if (useCursor) {
      const cur = await paymentLinkModel.findOne({
        where: { ...whereClause, link_id: Number(starting_after) },
        attributes: ['link_id', 'createdAt'],
      });
      const c = cur?.dataValues as { link_id: number; createdAt: Date } | undefined;
      if (c) {
        listWhere = {
          ...whereClause,
          [Op.or]: [
            { createdAt: { [Op.lt]: c.createdAt } },
            { createdAt: c.createdAt, link_id: { [Op.lt]: c.link_id } },
          ],
        };
      }
    }

    const rawLinks = await paymentLinkModel.findAll({
      where: listWhere,
      order: [['createdAt', 'DESC'], ['link_id', 'DESC']],
      ...(usePagination && { limit: limitNum + 1, ...(useCursor ? {} : { offset }) }),
    });
    const hasMore = usePagination && rawLinks.length > limitNum;
    const links = hasMore ? rawLinks.slice(0, limitNum) : rawLinks;
    ...
    if (usePagination) {
      const nextCursor = hasMore && formattedLinks.length > 0
        ? formattedLinks[formattedLinks.length - 1].link_id
        : null;
...
export const setCustomerEmail = async (
  ...
  try {
    ...
    const hasRef = typeof data === "string" && data.length > 0;
    const hasPaymentId = typeof payment_id === "string" && payment_id.length > 0;
    if (!hasRef && !hasPaymentId) {
      return errorResponseHelper(res, 400, "Missing payment reference.");
    }
    ...
    // The checkout session (Redis `customer-<ref>`) is a CONVENIENCE, not a hard
    // requirement. Buyers frequently type their email on the success screen
    // minutes after settlement, by which time the short-lived session may have
    // expired. If it's still present we merge the email in (so any in-flight
    // settlement receipt finds a recipient); if it's gone we fall back to the
    // database using the payment id the client passes. Either path NEVER
    // silently drops the buyer's email — the old code 404'd here and lost it.
    const key = hasRef ? "customer-" + data : null;
    const item = key ? ((await getRedisItem(key)) as Record<string, any> | null) : null;
    if (item && key) {
      await setRedisItem(key, { ...item, email: raw });
    }

    // ── Resolve the settled transaction from the DATABASE ────────────────────
    // The buyer's id can arrive as: payment_id from the client (addPayment's
    // transaction_id), or the session's payment_id / unique_tx_id /
    // current_payment_id. It may correspond to tbl_customer_transaction.unique_tx_id,
    // the on-chain transaction_reference, OR tbl_user_transaction.id. Match on
    // ALL of them so an id-shape mismatch can never hide the payment (which used
    // to leave anonymous buyers with no receipt AND an unlinked customer row).
    const candidateId = String(
      payment_id || item?.payment_id || item?.unique_tx_id || item?.current_payment_id || ""
    ).trim();
    let companyId = Number(item?.company_id) || 0;
    let tx: any = null;
    if (candidateId) {
      try {
        const rows = (await sequelize.query(
          `SELECT ct.customer_id, ct.company_id, ct.unique_tx_id, ct.language,
                  ct.base_amount, ct.base_currency, ct.paid_amount, ct.paid_currency,
                  ct.transaction_reference, ct.transaction_details, ct.status, ct."createdAt"
             FROM tbl_customer_transaction ct
             LEFT JOIN tbl_user_transaction ut ON ut.transaction_reference = ct.transaction_reference
            WHERE ct.unique_tx_id = :pid
               OR ct.transaction_reference = :pid
               OR ut.id = :pid
            ORDER BY ct."createdAt" DESC
            LIMIT 1`,
          { replacements: { pid: candidateId }, type: QueryTypes.SELECT }
        )) as any[];
        tx = rows?.[0] || null;
      } catch (lookupErr) {
        apiLogger.warn(`[setCustomerEmail] tx lookup failed: ${(lookupErr as Error).message}`);
      }
    }
    if (tx && !companyId) companyId = Number(tx.company_id) || 0;

    // ── Referral capture (best-effort) ───────────────────────────────────────
    // Link this real payer email to the (still-unassigned) transaction so the
    // invite cron (referralRewardMonitor) can reach them. A failure must NEVER
    // block saving/sending the receipt.
    try {
      if (companyId && tx && !tx.customer_id && tx.unique_tx_id) {
        ...
        const realCustomer = await findOrCreateEmailCustomer(
          ...
          String(item?.base_currency || tx.base_currency || "USD")
        );
        if (realCustomer?.customer_id) {
          ...
          await sequelize.query(
            ...
            {
              replacements: { cid: realCustomer.customer_id, pid: tx.unique_tx_id, company: companyId },
              type: QueryTypes.UPDATE,
            }
          );
          apiLogger.info(
            `[setCustomerEmail] referral: linked payment ${String(tx.unique_tx_id).slice(0, 8)}… to customer ${realCustomer.customer_id} (company ${companyId}) if unassigned`
          );
        }
      }
    } catch (e) {
    ...
    // ── Post-settlement receipt (best-effort) ────────────────────────────────
    // If the payment already settled (the success-screen "email me a receipt"
    // path), the settlement-time receipt never fired — no recipient was known
    // then. Send it now. Idempotent via the SAME Redis guard settlement uses
    // (`customer-receipt-email-<txRef>`). IMPORTANT: the guard is written only
    // AFTER a confirmed send, so a transient send failure stays RETRYABLE instead
    // of being permanently suppressed (the previous code set it before sending).
    let receiptSent = false;
    try {
      if (companyId && tx && String(tx.status).toLowerCase() === "successful") {
        const txRef = tx.transaction_reference || tx.unique_tx_id || candidateId;
        const dedupKey = `customer-receipt-email-${txRef}`;
        const already = (await getRedisItem(dedupKey)) as { sent?: boolean } | null;
        if (already?.sent) {
          receiptSent = true;
          apiLogger.info(
            `[setCustomerEmail] receipt already sent for tx ${String(txRef).slice(0, 10)}…, skipping`
          );
        } else {
          const companyRow = (await companyModel.findByPk(companyId)) as any;
          const created = tx.createdAt ? new Date(tx.createdAt) : new Date();
          const paymentSourceKey = classifyPaymentSource({
            linkId: (item as any)?.link_id ?? null,
            linkType: (item as any)?.link_type ?? null,
          });
          const buyAgainLink = await buildBuyAgainLink({
            linkId: (item as any)?.link_id ?? null,
            linkType: (item as any)?.link_type ?? null,
            parentLinkId: (item as any)?.parent_link_id ?? null,
            company: { handle: companyRow?.handle ?? null, creator_page_enabled: companyRow?.creator_page_enabled ?? null },
            buyerEmail: raw,
            buyerName: typeof item?.customer_name === "string" ? item.customer_name : null,
          });
          await sendCustomerPaymentConfirmationEmail(
            raw,
            typeof item?.customer_name === "string" ? item.customer_name : null,
            (await resolvePublicCompanyName(companyRow)) || "the merchant",
            `${tx.base_amount}`,
            tx.base_currency || "USD",
            txRef,
            (typeof item?.description === "string" ? item.description : null) || tx.transaction_details || null,
            ...
            parseFloat(tx.paid_amount || "0").toString(),
            tx.paid_currency || "",
            txRef,
            resolveCustomerLanguage({ transactionLang: tx.language, checkoutLang: (item as any)?.language }),
            undefined,
            item && shouldShowFeeSplit(companyRow, (item as any).fee_payer)
              ? resolveSettledBreakdown(item as any, tx.paid_currency || "")
              : null,
            companyRow?.photo || null,
            { companyId: companyId, ownerUserId: companyRow?.user_id ?? null },
            paymentSourceKey,
            buyAgainLink
          );
          // Guard AFTER a confirmed send so a throw above stays retryable.
          await setRedisItem(dedupKey, { sent: true, sentAt: new Date().toISOString() });
          await setRedisTTL(dedupKey, 86400);
          receiptSent = true;
          apiLogger.info(
            `[setCustomerEmail] post-settlement receipt emailed for tx ${String(txRef).slice(0, 10)}…`
          );
        }
      }
    } catch (receiptErr) {
      apiLogger.warn(
        `[setCustomerEmail] post-settlement receipt send failed (left retryable): ${(receiptErr as Error).message}`
      );
    }

    // If we could neither touch a live session nor resolve the payment, be honest
    // with the buyer rather than pretending we saved it.
    if (!item && !tx) {
      return errorResponseHelper(
        res,
        404,
        "We couldn't find this payment to send your receipt. Please contact the merchant."
      );
    }

    apiLogger.info(
      `[setCustomerEmail] processed (session=${item ? "yes" : "no"}, tx=${tx ? "yes" : "no"}, receiptSent=${receiptSent}) ref ${String(data || candidateId).slice(0, 10)}…`
    );
    return successResponseHelper(
      res,
      200,
      receiptSent ? "Receipt sent to your email." : "Receipt email saved.",
      { saved: true, receiptSent }
    );
  } catch (e) {
    handleControllerError(res, e, apiLogger, {});
  }
...
export const getCreatorProfile = async (req: express.Request, res: express.Response) => {
  try {
    ...
    // Visit analytics are recorded by the browser beacon
    // (POST /api/pay/creator/:handle/visit → creatorVisitTracking.ts), not here:
    // this endpoint is fetched by the Next.js server during SSR, so it never
    // sees the real visitor's IP / UA / referrer.
