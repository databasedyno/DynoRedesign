...
import { computeReceivedSplit } from "../checkoutMath";
    ...
    if (transactionId) {
      ...
      if (isFullPayment || webhook) {
        ...
        if (receivedUSD < Number(minForwarding)) {
          ...
          // ORIGINAL expected total (tempData.amount is only the remaining part after a partial payment)
          const expectedCrypto = originalExpected > 0 ? originalExpected : parseFloat(tempData?.amount || '0');
          const preCalcMerchantAmount = parseFloat(merchant_amount);
          
          if (expectedCrypto > 0) {
            // Underpaid → merchant share scales down proportionally. Overpaid → the quoted fee is
            // charged once and the ENTIRE excess is credited to the merchant (never kept as "fee").
            const split = computeReceivedSplit({ received: totalAmountReceived, expected: expectedCrypto, plannedMerchant: preCalcMerchantAmount });
            userAmountToSend = split.merchantAmount;
            adminAmountToSend = split.feeAmount;
            overpaymentExcessCrypto = split.overpaymentExcess;
            ...
            cronLogger.info(`[cryptoVerification] ${fee_payer === 'customer' ? 'CUSTOMER' : 'COMPANY'} PAYS FEES — RATIO-BASED DISTRIBUTION:
              ...
              - Payment ratio: ${toFixedStr(split.paymentRatio, 4)} (${split.paymentRatio > 1 ? 'overpaid → excess credited to merchant' : split.paymentRatio === 1 ? 'exact' : 'underpaid'})
              - Overpayment excess: ${toFixedStr(overpaymentExcessCrypto, 8)} ${tempCurrency}
              - Merchant: ${toFixedStr(userAmountToSend, 8)} ${tempCurrency} (pre-calc ${toFixedStr(preCalcMerchantAmount, 8)} + excess)
              - Admin (fee only): ${toFixedStr(adminAmountToSend, 8)} ${tempCurrency}`);
          } else {
        ...
        if (tempData.is_merchant_pool) {
          ...
          if (autoConvertEnabled && !adminTransferResult.transactionDetails) {
            ...
            // force: a real payment just landed — never let a stale dust deferral skip this sweep.
            merchantPoolService.sweepPoolAddress(sweepAddressId, { force: true }).then((sweepResult: { success?: boolean; skipped?: boolean; reason?: string; txId?: string }) => {
              if (sweepResult?.skipped || !sweepResult?.success) {
                cronLogger.warn(`[AutoConvert] ⚠️ Immediate sweep for address ID ${sweepAddressId} did NOT move funds (${sweepResult?.reason || "unknown"}) — conversion stays PENDING_DEPOSIT; stuck-conversion self-heal will retry`);
              } else {
                cronLogger.info(`[AutoConvert] Immediate sweep completed for address ID ${sweepAddressId} (tx ${sweepResult.txId || "n/a"})`);
              }
        ...
        // Overpayment: tell the merchant (email + in-app) and the admin that the buyer sent more than
        // due and that the excess is in the merchant's payout. Always — no threshold; deduped per payment.
        if (overpaymentExcessCrypto > 0) {
          ...
          persistTransition({
            ...
            from: PaymentState.PAYOUT_COMPLETE,
            to: PaymentState.PAYOUT_COMPLETE,
            event: "overpayment_credited",
            actor: "chain_verification",
            txId: transactionId,
            address,
            currency: tempCurrency,
            amount: overpaymentExcessCrypto,
            companyId: Number(customerData?.company_id) || null,
            metadata: { expected: originalExpected, received: Number(totalAmountReceived), excess_base: excessBase, base_currency: customerData?.base_currency || "USD", credited_to: "merchant" },
          }).catch(() => { /* journal is best-effort */ });
          notifyOverpayment({
            paymentId: overpaidPaymentId,
            companyId: customerData?.company_id || tempData?.company_id || null,
            txId: transactionId || tempData?.txId || null,
            currency: tempCurrency,
            amountReceived: toNumber(totalAmountReceived, 8),
            amountExpected: toNumber(originalExpected, 8),
            excessAmount: toNumber(overpaymentExcessCrypto, 8),
        ...
        // Classify HOW this payment was made (API / payment link / product order /
        // donation / tip) so the merchant + buyer emails can state the source.
        const paymentSourceKey = classifyPaymentSource({
          linkId: (customerData as any)?.link_id ?? (tempData as any)?.link_id ?? null,
        ...
        // (not the original coin) — userAmountToSend is 0 here, so reporting it
        // verbatim showed "received 0 <coin>". Show the USD value being converted
        // instead; the exact converted total arrives in a follow-up "Payout
        // complete" notification (see conversionService.monitorWithdrawals).
        ...
        await createNotification(
          ...
          mrNotifMessage,
          {
            amount: mrNotifAmount,
            currency: mrNotifCurrency,
            ...(autoConvertEnabled
        ...
        try {
          ...
          if (customerEmail && customerEmail.trim() !== "") {
            ...
            if (customerReceiptSent && customerReceiptSent.sent) {
              ...
              // Buyer Auto-Invite: "Buy from {Brand} again, faster" one-tap link.
              const buyAgainLink = await buildBuyAgainLink({
                linkId: (customerData as any)?.link_id ?? (tempData as any)?.link_id ?? null,
                linkType: (customerData as any)?.link_type ?? (tempData as any)?.link_type ?? null,
                parentLinkId: (customerData as any)?.parent_link_id ?? (tempData as any)?.parent_link_id ?? null,
                company: { handle: (company_data as any)?.handle ?? null, creator_page_enabled: (company_data as any)?.creator_page_enabled ?? null },
              ...
              await sendCustomerPaymentConfirmationEmail(
                ...
                shouldShowFeeSplit(company_data, tempData?.fee_payer)
                ...
                { companyId: company_data?.company_id ?? null, ownerUserId: company_data?.user_id ?? null },
                paymentSourceKey,
                buyAgainLink
              );
