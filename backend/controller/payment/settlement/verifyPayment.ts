...
import { probeUnconfirmedIncoming } from "../../../services/mempoolProbe";
  ...
  try {
    ...
    if (parsedState === PaymentState.PAYOUT_COMPLETE) {
      ...
      if (isSignificantOverpayment) {
        ...
        // Merchant/admin overpayment alert now fires from settlement (chainVerification) for EVERY
        // overpayment, not only when the buyer keeps this page open past the threshold.
      }
    ...
    if (parsedState === PaymentState.PENDING && !tempData?.txId) {
      // Payment initiated but no CONFIRMED transaction yet. Peek at the mempool / unconfirmed
      // layer so the buyer sees "Payment detected" as soon as they broadcast — a UI hint only;
      // settlement still waits for the chain-watcher webhook.
      const unconfirmed = await probeUnconfirmedIncoming(address, currency);
      if (unconfirmed.detected) {
        return successResponseHelper(res, 200, "Payment detected", {
          status: "pending",
          payment_status: "pending",
          unconfirmed: true,
          detected_via: unconfirmed.source,
          message: "Payment detected — waiting for network confirmation",
          txId: unconfirmed.txId || null,
          amount: unconfirmed.amount != null ? toFixedStr(unconfirmed.amount, 8) : undefined,
