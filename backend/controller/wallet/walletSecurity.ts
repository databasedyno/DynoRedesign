...
import { isWalletFrozen, performRevert } from "../../services/wallet/walletChangeAlert";
import { IUserType } from "../../utils/types";

/**
 * GET /api/wallet/security/status — read-only: is this account's payout-wallet
 * editing frozen (after a "this wasn't me" revert)? Powers /wallet/security.
 */
export const getWalletSecurityStatus = async (req: express.Request, res: express.Response) => {
  const userData = jwt.decode(res.locals.token) as IUserType;
  try {
    const freeze = await isWalletFrozen(userData.user_id);
