...
import { AUTO_CONVERT_SELECT_SQL, AUTO_CONVERT_ROW_KEYS, buildAutoConvertInfo } from "../../utils/autoConvertPayout";
...
export const getAllTransactions = async (
  ...
  try {
    ...
    const customer_data = await Promise.all(tempData.map(async (x: Record<string, unknown>) => {
      ...
      const rest: Record<string, unknown> = { ...restWithConvert };
      for (const k of AUTO_CONVERT_ROW_KEYS) delete rest[k];
      const auto_convert = buildAutoConvertInfo(x);
      ...
      return {
        ...rest,
        // Auto-converted payments: the merchant-facing "outgoing" hash is the Binance → merchant
        // payout, never the internal pool → Binance deposit transfer stored on the ledger row.
        outgoing_tx_hash: auto_convert ? auto_convert.payout_tx_hash : rest.outgoing_tx_hash,
