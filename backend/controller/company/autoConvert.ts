...
import { isCompanyOwnedByUser } from "../storefrontScope";
...
// Get single conversion detail by conversionId (owner-scoped)
const getConversionDetail = async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (!conversion || !(await isCompanyOwnedByUser(userData.user_id, Number(conversion.dataValues.company_id)))) {
...
// Retry a failed conversion (owner-scoped; E2 — merchant-facing "Retry")
