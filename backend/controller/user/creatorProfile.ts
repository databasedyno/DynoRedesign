...
import { isPlaceholderBrandName } from "../../helper/brandName";
...
export const updateCreatorProfile = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Can't enable the page without a handle — or while the brand still shows a
    // generated placeholder name (A1: the name is the page title, H1 and share text).
    if (updates.creator_page_enabled === true) {
      let curHandle: string | null = null;
      let curName: string | null = null;
      let ownerEmail: string | null = null;
      if (perCompany) {
        const c = await companyModel.findOne({ where: { company_id: activeCompanyId, user_id: userData.user_id }, attributes: ["handle", "company_name", "email"] });
        const d = (c?.dataValues || {}) as { handle?: string; company_name?: string; email?: string };
        curHandle = d.handle || null;
        curName = d.company_name || null;
        ownerEmail = d.email || null;
      } else {
        const cur = await userModel.findOne({ where: { user_id: userData.user_id }, attributes: ["handle", "name", "email"] });
        curHandle = cur?.dataValues?.handle || null;
        curName = cur?.dataValues?.name || null;
        ownerEmail = cur?.dataValues?.email || null;
      }
      if (!updates.handle && !curHandle) {
        return errorResponseHelper(res, 400, "Choose a handle before publishing your page");
      }
      const effectiveName = updates.name !== undefined ? (updates.name as string | null) : curName;
      if (isPlaceholderBrandName(effectiveName, [ownerEmail, userData.email])) {
        return errorResponseHelper(res, 400, "Set a display name before publishing your page — visitors see it as your page title");
      }
    }
  ...
  try {
    if (STOREFRONT_PER_COMPANY) {
      ...
      const company = await companyModel.findOne({
        ...
        attributes: [...STOREFRONT_COLUMNS, "company_name", "company_id", "email"],
      });
      ...
      const u = await userModel.findByPk(userData.user_id, { attributes: ["name", "email"] });
      const { email: companyEmail, ...rest } = d;
      return successResponseHelper(res, 200, "Creator profile", {
        ...rest,
        name: (d.company_name as string) || u?.dataValues?.name || null,
        // A1: lets the settings UI warn when the public name is still the generated placeholder.
        name_is_placeholder: isPlaceholderBrandName(d.company_name as string, [companyEmail as string, u?.dataValues?.email]),
