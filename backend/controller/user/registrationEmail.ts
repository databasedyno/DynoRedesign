...
export const registerEmailStep1 = async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (existing) {
      const sentLogin = await sendEmailOTP(emailLower, existing.dataValues.name || "there", { purpose: 'login' });
      if (!sentLogin) {
    ...
    const sent = await sendEmailOTP(emailLower, "there", { purpose: 'signup' });
    if (!sent) {
      return errorResponseHelper(res, 503, "Unable to send verification code. Please try again.");
    }
