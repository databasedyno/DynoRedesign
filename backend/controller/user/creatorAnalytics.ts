...
export const getCreatorStats = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (STOREFRONT_PER_COMPANY) {
      companyId = await resolveActiveCompanyId(req, userData.user_id);
      if (companyId) {
    ...
    // Supporters: distinct donors (by e-mail, else one per contribution) on this
    // storefront's completed tips / campaign contributions. Contributions are
    // child links of a 'donation' parent — tbl_user_transaction has no link_id,
    // so the previous join threw and silently reported 0.
    let supportersCount = 0;
    try {
      const companyScope =
        STOREFRONT_PER_COMPANY && companyId
          ? `AND COALESCE(parent.company_id, (SELECT MIN(company_id) FROM tbl_company WHERE user_id = :uid)) = :cid`
          : "";
      const rows = await sequelize.query(
        `SELECT COUNT(DISTINCT COALESCE(NULLIF(LOWER(c.email), ''), 'link:' || c.link_id::text)) AS c
         FROM tbl_payment_link c
         JOIN tbl_payment_link parent ON parent.link_id = c.parent_link_id
         WHERE parent.user_id = :uid
           AND parent.link_type = 'donation'
           AND c.link_type = 'contribution'
           AND LOWER(c.status) IN (:statuses)
           ${companyScope}`,
        {
          replacements: { uid: userData.user_id, cid: companyId, statuses: DONATION_COMPLETED_STATUSES },
          type: QueryTypes.SELECT,
        }
      ) as Array<{ c: string | number }>;
      supportersCount = Number(rows?.[0]?.c || 0);
    } catch (e) {
      userLogger.warn(`[creator/stats] supporters query failed: ${(e as Error)?.message}`);
    }
