...
export const login = async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (!userData || !isPasswordValid) {
      ...
      // ── Account soft-delete gate (defense-in-depth; finalizeLogin also gates) ──
  ...
  try {
    ...
    if (mobile) {
      ...
      if (userEmail) {
        ...
        const emailSent = await sendEmailOTP(userEmail, userName, { purpose: 'login' });
        if (emailSent) {
      ...
      if (userData?.dataValues) {
        const sent = await sendEmailOTP(email, userData.dataValues.name, { purpose: 'login' });
        if (sent) {
