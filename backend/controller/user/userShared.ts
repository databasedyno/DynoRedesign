...
export const finalizeLogin = async (
  ...
  // Account soft-delete gate (authoritative for ALL login paths — password,
  // email-OTP, social — since they all converge here). A scheduled-for-deletion
  // account cannot mint a new token or session until an admin restores it.
  if (isUserSoftDeleted(userData)) {
  ...
  // Create Session with Refresh Token (required for the token response)
  const sessionData = await createSession(userData.dataValues, req as any);

  // B1: Send the response IMMEDIATELY. None of the post-login bookkeeping
  // (geo-IP lookup, login-activity insert, notification email, last_login_ip
  // update) needs to precede the token response. Deferring it drops login
  // latency from ~1.3–1.5s to ~0.4–0.6s and makes login immune to ip-api.com
  // outages (which previously held the response hostage up to 3s).
  ...
  setImmediate(async () => {
    try {
          ...
          if (isBotUA || isInternalIp) {
            ...
            if (!alreadySeen) {
              ...
              sendNewDeviceAlertEmail(
                ...
                { ipAddress, device, browser, os, location, at: now, securityToken, lang: userData.dataValues.language }
      ...
      try {
        ...
        userLogger.error(`${logPrefix} Failed to update last_login_ip: ${updErr.message}`);
      }
    } catch (bgErr) {
      userLogger.error(`${logPrefix} Deferred post-login bookkeeping failed:`, bgErr);
    }
  });
};
 ...
 *
 * `opts.purpose` routes the code to the right localized OTP template (login /
 * signup / email-verify / password-reset / email-change / set-password) so a
 * password reset no longer reads like a "login" code. Defaults to login.
 */
export const sendEmailOTP = async (
  email: string,
  name: string,
  opts?: { purpose?: OtpPurpose; lang?: string }
): Promise<boolean> => {
  try {
    const randomNumberOTP = generateOtpCode();
    await sendPurposeOTPEmail(email, name, String(randomNumberOTP), opts?.purpose ?? 'login', opts?.lang);
