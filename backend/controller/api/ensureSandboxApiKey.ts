...
import { getErrorMessage, generateApiKeyName } from "../../helper";
...
export async function ensureSandboxApiKey(
  ...
  try {
    ...
    // Auto-provisioned: the token is hashed and discarded — the merchant rotates
    // ("Regenerate") from the dashboard to receive a key they can copy.
    const plaintextKey = generateApiKeyToken("development");
    ...
    await apiModel.create({
      ...
      key_hint: apiKeyHint(plaintextKey, "development"),
