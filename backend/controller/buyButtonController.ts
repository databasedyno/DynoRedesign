...
export const createBuyButton = async (
  ...
  try {
    ...
    const bbMin = getEffectiveMinOrderUsd("buy_button");
    if (price_type === "fixed") {
      if (typeof amount !== "number" || !Number.isFinite(amount) || amount < bbMin) {
        errorResponseHelper(res, 400, `amount must be a number ≥ ${bbMin} when price_type='fixed'`);
        return;
      }
      amt = amount;
    } else {
      ...
      if (min_amount !== undefined && min_amount !== null) {
        if (typeof min_amount !== "number" || !Number.isFinite(min_amount) || min_amount < bbMin) {
          errorResponseHelper(res, 400, `min_amount must be a number ≥ ${bbMin} when price_type='customer'`);
          return;
        }
        minA = min_amount;
      } else {
      ...
      if (max_amount !== undefined && max_amount !== null) {
        if (typeof max_amount !== "number" || !Number.isFinite(max_amount) || max_amount <= (minA || bbMin)) {
