  ...
  try {
    ...
    const countQuery = `
      SELECT 
        ...
        COUNT(*) FILTER (WHERE ut.status = 'underpaid') as underpaid_count,
    ...
    const underpaidCount = parseInt(String(stats.underpaid_count || '0'));
...
const getActionCounts = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    const companyScopeSql = company_id
      ? `= :companyId`
      : `IN (SELECT company_id FROM tbl_company WHERE user_id = :userId AND deleted_at IS NULL)`;
