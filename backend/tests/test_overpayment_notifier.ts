...
async function run() {
  ...
  assert(!!admin && /credited to the merchant/i.test(admin.body), "admin email states excess credited to the merchant");
  assert(!!merchant && /never keeps an overpayment/i.test(merchant.body), "merchant email states the excess is in their payout");
