...
const callUrlWithPayload = async (
  ...
  try {
    ...
    // Serialize the body ONCE and send those exact bytes, so the v2 signature
    // below is verifiable against the raw request body (API review §5.1.1).
    ...
    // (API review §5.1.2). There is NO shared-default fallback any more: signing
    // under a guessable shared secret is worse than not signing, because it looks
    // verified. Endpoints without a secret receive the payload unsigned.
    //   • X-DynoPay-Signature    (v1, legacy)  — HMAC over the re-serialised object
    //   • X-Dynopay-Signature-V2 (recommended) — Stripe-style, HMAC over
    //     "<t>.<rawBody>" so any language can verify against the raw bytes.
    if (webhookSecret) {
      headers['X-DynoPay-Signature'] = generateWebhookSignature(signaturePayload, webhookSecret);
      const v2parts = [`v1=${hmacSha256Hex(`${timestamp}.${rawBody}`, webhookSecret)}`];
      // Zero-downtime rotation: during the 24h grace after a merchant rotates
      // their signing secret, the COMPANY URL is also signed with the previous
      // secret (extra `v1=`), so a merchant who has not yet swapped their stored
      // secret still verifies. Comma-delimited v1 entries are Standard-Webhooks/
      // Stripe compatible — any verifier scans every v1= for a match.
      if (isCompanyUrl && companyId) {
        try {
          const sequelize = require('../utils/dbInstance').default;
          const prevRows = await sequelize.query(
            `SELECT webhook_secret_previous FROM tbl_company
              WHERE company_id = :cid
                AND webhook_secret_previous IS NOT NULL
                AND webhook_secret_previous_expires_at IS NOT NULL
                AND webhook_secret_previous_expires_at > NOW() LIMIT 1`,
            { replacements: { cid: companyId }, type: QueryTypes.SELECT }
          ) as Array<{ webhook_secret_previous: string }>;
          const prev = prevRows[0]?.webhook_secret_previous;
          if (prev && prev !== webhookSecret) {
            v2parts.push(`v1=${hmacSha256Hex(`${timestamp}.${rawBody}`, prev)}`);
          }
        } catch (rotErr) {
          webhookLogs.warn(`[callMerchantWebhook] rotation co-sign lookup failed for company_id=${companyId}: ${(rotErr as Error).message}`);
        }
      }
      headers['X-Dynopay-Signature-V2'] = `t=${timestamp},${v2parts.join(',')}`;
    }
    
    webhookLogs.info(`[callMerchantWebhook] Sending ${urlType} ${eventData.event} to ${url}`);
    webhookLogs.info(`[callMerchantWebhook] Signature included: ${webhookSecret ? 'yes (v1 + v2)' : 'no (endpoint has no secret)'}`);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const { response, finalUrl, redirectChain } = await postWithSafeRedirects(url, rawBody, headers);
...
/**
 * Redeliver a previously-logged webhook to a specific URL (API review §5.1.4 —
 * POST /api/user/events/:id/resend). Reuses the exact delivery path so the
 * resend carries a fresh webhook_id, timestamp and signature. Never throws.
 */
const redeliverWebhook = (
  url: string,
  eventData: Record<string, unknown>,
  secret: string | null,
  companyId: number | null,
