...
1. **Check Webhook History**
   ```bash
   curl -X GET "https://dynopay.com/api/company/webhook-history/{company_id}" \
     -H "Authorization: Bearer YOUR_TOKEN"
   ```

2. **Send Test Webhook**
   ```bash
   curl -X POST "https://dynopay.com/api/company/webhook-test/{company_id}" \
     -H "Authorization: Bearer YOUR_TOKEN"
   ```
...
| `X-DynoPay-Signature` | **(v1, legacy)** HMAC-SHA256 of a re-serialised body — kept for backward compatibility. |
| `X-Dynopay-Signature-V2` | **(v2, recommended)** `t=<unix>,v1=<hex>`, where hex = `HMAC-SHA256(secret, "<t>.<rawBody>")` over the **exact raw body**. Sent only when a `webhook_secret` is configured. |
...
The `webhook_secret` is **optional**. If you don't configure a secret, webhooks are still sent but without any signature header.

> **Use v2.** `X-Dynopay-Signature-V2` is signed over the exact bytes on the wire, so you can verify it in any language without re-serialising the parsed JSON (which the legacy v1 header requires and which breaks whenever a JSON library reorders keys or formats numbers differently). Both headers are sent during the migration window; v1 will be removed after it closes.

### Node.js Example (v2 — recommended)
...
// Express.js — capture the raw body so the bytes are byte-identical to what was signed.
app.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  const rawBody = req.body.toString();
  const v2 = req.headers['x-dynopay-signature-v2'];

  if (v2 && !verifyWebhookV2(rawBody, v2, WEBHOOK_SECRET)) {
...
def verify_webhook_v2(raw_body: bytes, header_v2: str, secret: str, tolerance: int = 300) -> bool:
    parts = dict(p.split("=", 1) for p in header_v2.split(","))
    t = int(parts.get("t", 0))
...
curl -X PUT "https://dynopay.com/api/company/webhook-settings/{company_id}" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "https://dynopay.com/api/company/webhook-settings/{company_id}" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
# Filter by status
curl -X GET "https://dynopay.com/api/company/webhook-history/{company_id}?status=failed" \
  -H "Authorization: Bearer YOUR_TOKEN"

# Filter by event type
curl -X GET "https://dynopay.com/api/company/webhook-history/{company_id}?event_type=payment.confirmed" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
curl -X GET "https://dynopay.com/api/company/webhook-history/{company_id}/detail/{log_id}" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
# Last 7 days (default)
curl -X GET "https://dynopay.com/api/company/webhook-stats/{company_id}" \
  -H "Authorization: Bearer YOUR_TOKEN"

# Custom period (up to 30 days)
curl -X GET "https://dynopay.com/api/company/webhook-stats/{company_id}?days=14" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
Each delivery attempt makes up to 3 fast in-process tries (1 s, 2 s, 4 s) to ride out a
momentary blip. If the attempt still fails, the event is queued for **durable redelivery on a
fixed schedule** so a deploy or a short outage never loses it:

**1 min → 5 min → 30 min → 2 h → 6 h → 12 h → 24 h** (7 retries over ~24 hours).

After the final attempt the event is marked failed (and can still be re-sent manually, below).
...
### Reconciliation — catch up on missed events

If your server was down, you don't have to poll `getPaymentStatus` per payment:

- `GET /api/user/events?limit=&starting_after=&type=&status=` — cursor-paginated list of the
  events we've sent to your endpoints (`id`, `event`, `url`, `status`, `response_status`,
  `attempts`, timestamps). Page forward with the returned `next_cursor` until `has_more` is false.
- `POST /api/user/events/{id}/resend` — re-deliver one event to its original URL with a fresh
  `webhook_id`, timestamp and signature. Both use your `x-api-key` and are scoped to your brand.
