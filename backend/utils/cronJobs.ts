    ...
    try {
      ...
      for (const user of usersWithWeeklySummary) {
        try {
          ...
          // Weekly-summary EMAIL retired to remove a duplicate: the weekly
          // payout digest (server.ts → sendPayoutDigestsToAll) is now the single
          // weekly email. The in-app weekly-summary notification above is kept.
