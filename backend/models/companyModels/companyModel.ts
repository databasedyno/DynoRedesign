...
const companyModel = sequelize.define(
  "Company",
  {
    ...
    // Two-secret rotation (zero-downtime): when a merchant rotates their signing
    // secret we keep the OLD one here with a 24h expiry. During that window the
    // company URL is signed with BOTH secrets (extra v1= in X-Dynopay-Signature-V2)
    // so a merchant who has not yet swapped their stored secret still verifies.
    webhook_secret_previous: {
      type: DataTypes.STRING(100),
      allowNull: true,
      comment: "Previous signing secret, still honoured until webhook_secret_previous_expires_at (rotation grace)",
    },
    webhook_secret_previous_expires_at: {
      type: DataTypes.DATE,
      allowNull: true,
      comment: "When the previous signing secret stops being co-signed (24h after rotation)",
    },
    ...
    // B12: NULL = auto (show the merchant/Dynopay split only when the customer pays the fee), true = always, false = never.
    show_fee_split_to_customers: {
      type: DataTypes.BOOLEAN,
      allowNull: true,
      defaultValue: null,
      comment: "Whether buyers see 'Merchant receives / Dynopay fee' on checkout + receipts. NULL = only when the customer pays the fee.",
    },
    // Phase 1b (2026-09-13, migration 0026_company_min_order_usd): per-brand
    // minimum order amount in USD. NULL = inherit the platform surface defaults
    // (services/checkout/orderMinimums.ts). A merchant may RAISE it (never below).
    // Enforced universally at pay time (createCryptoPayment) and surfaced to the
    // checkout so un-payable coins/amounts are greyed up-front.
    min_order_usd: {
      type: DataTypes.DECIMAL(10, 2),
      allowNull: true,
      defaultValue: null,
      comment: "Per-brand minimum order (USD). NULL = platform defaults. Merchant can only raise it.",
    },
    ...
    // ── 7-DAY SOFT DELETE (migration 012) ──────────────────────────────────
    // Brand deletion is now reversible for 7 days. `deleted_at` (the paranoid
    // column) is set on delete so ALL ORM reads exclude the brand immediately;
    // `scheduled_purge_at` = deleted_at + 7d is when the daily cron hard-purges
    // it; `deleted_by` records the account user who triggered the delete.
    deleted_by: {
      type: DataTypes.INTEGER,
      allowNull: true,
      comment: "user_id that soft-deleted this brand (7-day grace period).",
    },
    scheduled_purge_at: {
      type: DataTypes.DATE,
      allowNull: true,
      comment: "When a soft-deleted brand becomes eligible for permanent purge (deleted_at + 7d).",
    },
  ...
  },
  {
    ...
    // Paranoid soft-delete: destroy() sets `deleted_at` and every find/count/
    // association automatically excludes soft-deleted brands. Use
    // `{ paranoid: false }` (or restore()) in the admin restore/purge paths.
    paranoid: true,
