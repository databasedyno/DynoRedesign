...
const qaItemMetaModel = sequelize.define(
  "tbl_qa_item_meta",
  {
    // One row per test item (catalog case or journey). item_key === `${sectionId}::${caseId}`.
    item_key: {
      type: DataTypes.STRING(255),
      primaryKey: true,
    },
    section_id: { type: DataTypes.STRING(120), allowNull: true },
    section_title: { type: DataTypes.STRING(255), allowNull: true },
    case_title: { type: DataTypes.STRING(500), allowNull: true },
    // Who has claimed this item.
    assignee: { type: DataTypes.STRING(160), allowNull: true },
    // Release sign-off.
    signed_off: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
    signed_off_by: { type: DataTypes.STRING(160), allowNull: true },
    signed_off_at: { type: DataTypes.DATE, allowNull: true },
    updated_at: { type: DataTypes.DATE, allowNull: false, defaultValue: DataTypes.NOW },
  },
  {
    tableName: "tbl_qa_item_meta",
    timestamps: false,
  }
);
...
export function ensureQaTables(): Promise<void> {
  if (!ensurePromise) {
    ensurePromise = (async () => {
      ...
      await qaItemMetaModel.sync({ alter: false });
      apiLogger.info("[QA] quality-center tables ensured (tbl_qa_comment, tbl_qa_custom_item, tbl_qa_item_meta)");
    })().catch((e) => {
...
export { qaCommentModel, qaCustomItemModel, qaItemMetaModel };
