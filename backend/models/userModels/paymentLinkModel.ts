...
const paymentLinkModel = sequelize.define(
  "Payment_Link",
  {
    ...
    // Buyer's language captured when they open this link's checkout (ISO 639-1).
    // Persisted so payment-link REMINDER emails follow the BUYER instead of the
    // merchant. Column pre-exists in the DB (was previously unused).
    default_language: {
      type: DataTypes.STRING,
      allowNull: true,
    },
