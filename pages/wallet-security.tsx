...
const WalletSecurityPage: NextPageWithLayout = () => {
  ...
  const body =
    status === "loading"
      ...
      : status === "success"
        ? `We've undone the recent payout wallet change${networks.length > 1 ? "s" : ""}${
            networks.length ? ` (${networks.join(", ")})` : ""
          } and locked wallet changes on your account. To make new wallet changes, contact support so we can confirm it's really you.`
        : status === "expired"
          ? "This security link has already been used or has expired. If you still think something's wrong, contact support right away."
          : "We couldn't complete this action. Please try again, or contact support if the problem continues.";

  return (
    <>
      <Head>
        <title>Account security · DynoPay</title>
