...
type PublicReceipt = {
  ...
  breakdown: { merchantReceives: string; platformFee: string; feePayer: 'customer' | 'company'; feeNote: string } | null
      ...
      <Box sx={{ minHeight: '100vh', bgcolor: theme.palette.background.default, py: { xs: 3, sm: 6 }, px: 2 }} data-testid="public-receipt-page">
        <Box sx={{ maxWidth: 560, mx: 'auto' }}>
          ...
          <Box sx={{ border: `1px solid ${border}`, borderRadius: '16px', bgcolor: theme.palette.background.paper, overflow: 'hidden' }}>
            ...
            {/* Breakdown */}
            {receipt.breakdown && (
              <Box sx={{ px: { xs: 2.5, sm: 3.5 }, py: 2.5, borderBottom: `1px solid ${border}` }} data-testid="public-receipt-breakdown">
                <Typography sx={{ fontSize: 13, fontWeight: 700, color: theme.palette.text.primary, mb: 0.5 }}>{L.breakdown}</Typography>
                <Row label={L.youPaid} mono>{receipt.cryptoAmount ? `${trimCrypto(receipt.cryptoAmount)} ${receipt.coinSymbol}` : `${receipt.amount} ${receipt.currency}`}</Row>
                <Row label={L.merchantReceives} mono>{receipt.breakdown.merchantReceives}</Row>
                <Row label={L.platformFee} mono>
                  {receipt.breakdown.platformFee}
                  <Box component="span" sx={{ fontFamily: 'inherit', fontSize: 11.5, color: muted, ml: 0.75 }}>({receipt.breakdown.feeNote})</Box>
                </Row>
              </Box>
            )}
