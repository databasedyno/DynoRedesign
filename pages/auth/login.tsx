...
// Post-login brand splash — a pulsing Dynopay logo shown while we hand off from
// the login screen to the dashboard, so the transition never hard-cuts.
...
export default function Login() {
  ...
  // Brand splash shown during the login -> dashboard hand-off (see the
  // success effect below). Kept mounted until this page unmounts (dashboard
  // renders), so the logo bridges the whole transition.
  ...
  useEffect(() => {
    if ((userState.email || userState.name) && !isPasswordRecoveryMode) {
      ...
      // Show the pulsing Dynopay logo splash while we hand off to the dashboard.
      setRedirecting(true);
      setTimeout(() => {
  ...
  return (
    ...
    {redirecting && (
      <Box
        data-testid="login-redirect-splash"
        role="status"
        ...
        sx={{
          ...
          backgroundColor:
            mounted && theme.palette.mode === "dark"
              ? "rgba(8, 8, 10, 0.92)"
      ...
      >
        <Box
          component="img"
          src={
            ((mounted && theme.palette.mode === "dark" ? WhiteLogo : Logo) as { src?: string })?.src ||
            ((mounted && theme.palette.mode === "dark" ? WhiteLogo : Logo) as unknown as string)
          }
          alt="Dynopay"
          draggable={false}
          sx={{
