...
const ShopPage: NextPageWithLayout<ShopPageProps> = ({ merchant, products, siteUrl, metaLang, verified }) => {
  ...
  React.useEffect(() => {
    sendCreatorVisitBeacon(merchant.handle, "shop");
  }, [merchant.handle]);
  ...
  return (
    <>
      <Head>
        ...
        {/* D7: empty shops are not worth indexing under the brand */}
        {products.length === 0 && <meta name="robots" content="noindex, follow" />}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  if (!getRuntimeFlags().enableProductCatalog) {
