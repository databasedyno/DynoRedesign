export const amlPolicy = [
  {
    title: "1. Purpose of the Policy",
    description:
      "Our goal is to ensure that Dynopay is not used to facilitate illegal financial activity. We follow international AML standards and apply reasonable risk-based measures across our platform.",
    bulletPoints: [],
    footer:
      "",
  },
  {
    title: "2. Scope",
    description: "This Policy applies to:",
    bulletPoints: [
      "All users and companies using the Service",
      "All payment link transactions",
      "All API-generated transactions",
      "All connected cryptocurrency wallets",
    ],
    footer:
      "We do not store or control user funds, but we monitor platform activity for AML compliance.",
  },
  {
    title: "3. KYC Requirements",
    description:
      "Depending on risk level and applicable regulations, we may request:",
    bulletPoints: [
      "Full name",
      "Company information",
      "Valid identification documents",
      "Proof of address",
      "Information about business activity and transaction purpose",
    ],
    footer: "We may suspend service until required information is provided.",
  },
  {
    title: "4. Wallet & Transaction Monitoring",
    description:
      "We use automated and manual checks to identify suspicious activity, including:",
    bulletPoints: [
      "High-risk wallet addresses",
      "Known illicit sources (sanctioned, hacked, mixer-related)",
      "Unusual transaction patterns",
      "High-risk jurisdictions",
      "Structuring or repeated failed attempts",
    ],
    footer:
      "We may request clarification or supporting documentation from users.",
  },
  {
    title: "5. Prohibited Use",
    description: "You may not use the Service for:",
    bulletPoints: [
      "Money laundering or terrorism financing",
      "Fraud, scams, or phishing",
      "Dark-web market activity",
      "Obscured mixing/tumbling",
      "Sanctioned individuals or entities",
      "Illegal goods or services",
    ],
    footer: "We may report prohibited behavior to relevant authorities.",
  },
  {
    title: "6. Record Keeping",
    description: "To comply with AML requirements, we may securely store:",
    bulletPoints: [
      "KYC information",
      "Transaction metadata",
      "Internal compliance notes",
      "Risk assessments",
      "Records may be kept for up to the legally required duration.",
    ],
    footer: "",
  },
  {
    title: "7. Reporting Obligations",
    description:
      "We may submit reports to regulators or law enforcement if we identify:",
    bulletPoints: [
      "Suspicious transactions",
      "Attempts to evade compliance checks",
      "Fraudulent or criminal activity",
      "Users are not notified when reporting is legally restricted.",
    ],
    footer: "",
  },
  {
    title: "8. Staff Training & Compliance Oversight",
    description: "Our team receives ongoing training in:",
    bulletPoints: [
      "AML requirements",
      "Risk detection",
      "Handling of suspicious activity",
    ],
    footer:
      "A designated compliance officer oversees implementation of this Policy.",
  },
  {
    title: "9. Updates to This Policy",
    description: "We may update this AML Policy periodically. Changes take effect once published on our website.",
    bulletPoints: [],
    footer: "",
  },
];
