...
install_deps() {
  ...
  # --prefer-offline: reuse the warm yarn cache and skip the flaky registry fetch
  # (removes the network-retry stall + the incomplete-first-pass double install).
  (cd "$dir" && flock /tmp/dynopay-yarn-install.lock yarn install --non-interactive --production=false --prefer-offline --network-concurrency 16 >"/tmp/yarn-$label.log" 2>&1)
  if [ ! -x "$probe" ]; then
    # yarn says "already up-to-date" for a partially-present tree; --check-files repairs it.
    (cd "$dir" && flock /tmp/dynopay-yarn-install.lock yarn install --non-interactive --production=false --prefer-offline --check-files >>"/tmp/yarn-$label.log" 2>&1)
  fi
  if [ -x "$probe" ]; then
    ok "$label installed"
  else
