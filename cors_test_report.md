...
## Backend URL: https://config-preview-7.preview.emergentagent.com
...
| 5 | https://config-preview-7.preview.emergentagent.com | ALLOWED | `*` (wildcard) | ⚠️ ALLOWED (wrong reason) |
...
### Test CORS Preflight
```bash
curl -v -X OPTIONS "https://config-preview-7.preview.emergentagent.com/api/csrf-token" \
...
```bash
curl -v -X GET "https://config-preview-7.preview.emergentagent.com/api/csrf-token" \
  -H "Origin: https://dynopay.com"
```
