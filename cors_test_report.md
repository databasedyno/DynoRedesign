...
## Backend URL: https://payment-config-dev.preview.emergentagent.com
...
| 5 | https://payment-config-dev.preview.emergentagent.com | ALLOWED | `*` (wildcard) | ⚠️ ALLOWED (wrong reason) |
...
### Test CORS Preflight
```bash
curl -v -X OPTIONS "https://payment-config-dev.preview.emergentagent.com/api/csrf-token" \
...
```bash
curl -v -X GET "https://payment-config-dev.preview.emergentagent.com/api/csrf-token" \
  -H "Origin: https://dynopay.com"
```
