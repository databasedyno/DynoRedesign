...
## Backend URL: https://repo-link-setup.preview.emergentagent.com
...
| 5 | https://repo-link-setup.preview.emergentagent.com | ALLOWED | `*` (wildcard) | ⚠️ ALLOWED (wrong reason) |
...
### Test CORS Preflight
```bash
curl -v -X OPTIONS "https://repo-link-setup.preview.emergentagent.com/api/csrf-token" \
...
```bash
curl -v -X GET "https://repo-link-setup.preview.emergentagent.com/api/csrf-token" \
  -H "Origin: https://dynopay.com"
```
