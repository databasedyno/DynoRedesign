...
## Backend URL: https://crypto-checkout-init-1.preview.emergentagent.com
...
| 5 | https://crypto-checkout-init-1.preview.emergentagent.com | ALLOWED | `*` (wildcard) | ⚠️ ALLOWED (wrong reason) |
...
### Test CORS Preflight
```bash
curl -v -X OPTIONS "https://crypto-checkout-init-1.preview.emergentagent.com/api/csrf-token" \
...
```bash
curl -v -X GET "https://crypto-checkout-init-1.preview.emergentagent.com/api/csrf-token" \
  -H "Origin: https://dynopay.com"
```
